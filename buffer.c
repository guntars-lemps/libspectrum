...
void libspectrum_buffer_reallocate(libspectrum_buffer *buffer, const size_t new_size)
{
...
libspectrum_buffer *libspectrum_buffer_alloc(void)
{
...
void libspectrum_buffer_free(libspectrum_buffer *buffer)
{
...
int libspectrum_buffer_is_empty(const libspectrum_buffer *buffer)
{
    return buffer->bytes_used == 0;
}


int libspectrum_buffer_is_not_empty(const libspectrum_buffer *buffer)
{
    return buffer->bytes_used > 0;
}


void libspectrum_buffer_write_byte(libspectrum_buffer *buffer, const libspectrum_byte data)
{
    libspectrum_buffer_write(buffer, &data, sizeof(libspectrum_byte)) ;
}


void libspectrum_buffer_write_word(libspectrum_buffer *buffer, const libspectrum_word data)
{
...
void libspectrum_buffer_write_dword(libspectrum_buffer *buffer, const libspectrum_dword data)
{
...
void libspectrum_buffer_write_buffer(libspectrum_buffer *dest, const libspectrum_buffer *src)
{
    if (src) {
...
static void reallocate_to_new_size(libspectrum_buffer *buffer, const size_t size)
{
...
void libspectrum_buffer_write(libspectrum_buffer *buffer, const void* data, const size_t size)
{
...
void libspectrum_buffer_set(libspectrum_buffer *buffer, libspectrum_byte value, const size_t size)
{
    reallocate_to_new_size(buffer, size);
...
size_t libspectrum_buffer_get_data_size(const libspectrum_buffer *buffer)
{
    return buffer ? buffer->bytes_used : 0;
}


libspectrum_byte *libspectrum_buffer_get_data(const libspectrum_buffer *buffer)
{
    return buffer ? buffer->buffer : NULL;
}


void libspectrum_buffer_clear(libspectrum_buffer *buffer)
{
    if (buffer) buffer->bytes_used = 0;
}


void libspectrum_buffer_append(libspectrum_byte **buffer,
                               size_t *length,
