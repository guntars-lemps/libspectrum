...
static int libspectrum_sna_read_header(const libspectrum_byte *buffer, size_t buffer_length, libspectrum_snap *snap);
static int libspectrum_sna_read_data(const libspectrum_byte *buffer, size_t buffer_length, libspectrum_snap *snap);
static int libspectrum_sna_read_128_header(const libspectrum_byte *buffer, size_t buffer_length, libspectrum_snap *snap);
static int libspectrum_sna_read_128_data(const libspectrum_byte *buffer, size_t buffer_length, libspectrum_snap *snap);

static void write_header(libspectrum_buffer *buffer, libspectrum_snap *snap, libspectrum_word sp);
static libspectrum_error write_48k_sna(libspectrum_buffer *buffer, libspectrum_snap *snap, libspectrum_word *new_sp);
static void write_128k_sna(libspectrum_buffer *buffer, libspectrum_snap *snap);
static void write_page(libspectrum_buffer *buffer, libspectrum_snap *snap, int page);
...
libspectrum_error internal_sna_read(libspectrum_snap *snap, const libspectrum_byte *buffer, size_t buffer_length)
{
    ...
    error = libspectrum_sna_read_data(&buffer[LIBSPECTRUM_SNA_HEADER_LENGTH],
                                      buffer_length - LIBSPECTRUM_SNA_HEADER_LENGTH,
                                      snap);
    if (error != LIBSPECTRUM_ERROR_NONE) {
        return error;
    }
...
static int identify_machine(size_t buffer_length, libspectrum_snap *snap)
{
    switch (buffer_length) {
        ...
        case 147487:
            ...
            break;
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_sna_identify: unknown length");
...
static int libspectrum_sna_read_data(const libspectrum_byte *buffer, size_t buffer_length, libspectrum_snap *snap)
{
    ...
    if (buffer_length < 0xc000) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_sna_read_data: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    switch (libspectrum_snap_machine(snap)) {
        ...
        case LIBSPECTRUM_MACHINE_48:
            ...
            if ((sp < 0x4000) || (sp == 0xffff)) {
                libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_sna_read_data: SP invalid (0x%04x)", sp);
                return LIBSPECTRUM_ERROR_CORRUPT;
            }
        ...
        case LIBSPECTRUM_MACHINE_PENT:
            ...
            error = libspectrum_sna_read_128_header((buffer + 0xc000), (buffer_length - 0xc000), snap);
            if (error != LIBSPECTRUM_ERROR_NONE) {
                return error;
            }
            ...
            if ((page == 5) || (page == 2)) {
                if (memcmp(libspectrum_snap_pages(snap, page), &buffer[0x8000], 0x4000)) {
                    libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT,
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "libspectrum_sna_read_data: unknown machine");
...
{
    if (buffer_length < 4) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_sna_read_128_header: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
...
static int libspectrum_sna_read_128_data(const libspectrum_byte *buffer, size_t buffer_length, libspectrum_snap *snap)
{
    ...
    for (i = 0; i <= 7; i++) {
        ...
        if ((i == 2) || (i == 5) || (i == page)) {
            continue; // Already got this page
        }

        // Check we've still got some data to read
        if (buffer_length < 0x4000) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_sna_read_128_data: not enough data in buffer");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
...
libspectrum_error libspectrum_sna_write(libspectrum_buffer *buffer,
                                        int *out_flags,
                                        ...
                                        int in_flags GCC_UNUSED)
{
    ...
    // Minor info loss already due to things like tstate count, halted state, etc which are not stored in .sna format
