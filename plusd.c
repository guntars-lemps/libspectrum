...
static libspectrum_error libspectrum_plusd_read_header(const libspectrum_byte *buffer,
...
static libspectrum_error libspectrum_plusd_read_data(const libspectrum_byte *buffer, libspectrum_snap *snap)
{
    ...
    // We must have 0x4000 <= SP <= 0xfffa so we can rescue the stacked registers
    if ((sp < 0x4000) || (sp > 0xfffa)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_plusd_read_data: SP invalid (0x%04x)", sp);
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    libspectrum_snap_set_pc(snap, (readbyte(snap, (sp + 4)) + (readbyte(snap, (sp + 5)) * 0x100)));
