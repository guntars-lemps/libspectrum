...
#ifndef HAVE_LIB_GLIB // Use this iff we're not using the `proper' glib
#include <string.h>
...
static void g_hash_nodes_destroy(GHashNode *hash_node, GFreeFunc key_destroy_func, GFreeFunc value_destroy_func)
{
    if (hash_node) {
...
static GHashNode **g_hash_table_lookup_node(GHashTable *hash_table, gconstpointer key)
{
    ...
    node = &hash_table->nodes[(*hash_table->hash_func)(key) % HASH_TABLE_SIZE];
...
restart:
        ...
        for (node = hash_table->nodes[i]; node; prev = node, node = node->next) {
            if ((*func)(node->key, node->value, user_data)) {
                ...
                if (prev) {
                    prev->next = node->next;
                    g_hash_node_destroy(node, hash_table->key_destroy_func, hash_table->value_destroy_func);
                    node = prev;
                } else {
