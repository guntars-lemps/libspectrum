...
enum {
    ...
    // The first extension ID; anything here or greater applies to both v2 and v3 files
    Z80_MACHINE_FIRST_EXTENSION = Z80_MACHINE_PLUS3
};

// Constants representing a Sinclair Interface 2 port 1 joystick as a .z80 user defined joystick
...
static libspectrum_error read_header(const libspectrum_byte *buffer,
                                     libspectrum_snap *snap,
                                     const libspectrum_byte **data,
                                     int *version,
                                     int *compressed);
static libspectrum_error get_machine_type(libspectrum_snap *snap,
...
static libspectrum_error get_machine_type_v3(libspectrum_snap *snap, libspectrum_byte type, libspectrum_byte mgt_type);
static libspectrum_error get_machine_type_extension(libspectrum_snap *snap, libspectrum_byte type);
static libspectrum_error get_joystick_type(libspectrum_snap *snap,
...
static libspectrum_error get_joystick_type_v3(libspectrum_snap *snap,
                                              const libspectrum_byte *custom_joystick_base,
                                              libspectrum_byte type);
static libspectrum_error read_blocks(const libspectrum_byte *buffer,
                                     size_t buffer_length,
                                     libspectrum_snap *snap,
                                     int version,
                                     int compressed);
static libspectrum_error read_slt(libspectrum_snap *snap,
                                  const libspectrum_byte **next_block,
                                  const libspectrum_byte *end);
static libspectrum_error read_block(const libspectrum_byte *buffer,
                                    libspectrum_snap *snap,
                                    const libspectrum_byte **next_block,
                                    const libspectrum_byte *end,
                                    int version,
                                    int compressed);
static libspectrum_error read_v1_block(const libspectrum_byte *buffer,
                                       int is_compressed,
                                       libspectrum_byte **uncompressed,
                                       const libspectrum_byte **next_block,
                                       const libspectrum_byte *end);
static libspectrum_error read_v2_block(const libspectrum_byte *buffer,
                                       libspectrum_byte **block,
                                       size_t *length,
                                       int *page,
                                       const libspectrum_byte **next_block,
                                       const libspectrum_byte *end);

static libspectrum_error write_header(libspectrum_buffer *buffer, int *flags, libspectrum_snap *snap);
static void write_base_header(libspectrum_buffer *buffer, int *flags, libspectrum_snap *snap);
static libspectrum_error write_extended_header(libspectrum_buffer *buffer, int *flags, libspectrum_snap *snap);
static void write_pages(libspectrum_buffer *buffer, libspectrum_snap *snap, int compress);
static void write_page(libspectrum_buffer *buffer, int page_num, libspectrum_byte *page, int compress);
static void write_slt(libspectrum_buffer *buffer, libspectrum_snap *snap);
static void write_slt_entry(libspectrum_buffer *buffer,
                            libspectrum_word type,
                            libspectrum_word id,
                            libspectrum_dword slt_length);

static void compress_block(libspectrum_byte **dest,
                           size_t *dest_length,
                           const libspectrum_byte *src,
                           size_t src_length);
static void uncompress_block(libspectrum_byte **dest,
                             size_t *dest_length,
                             const libspectrum_byte *src,
                             size_t src_length);
...
libspectrum_error internal_z80_read(libspectrum_snap *snap, const libspectrum_byte *buffer, size_t buffer_length)
{
    ...
    error = read_blocks(data, (buffer_length - (data - buffer)), snap, version, compressed);
    if (error != LIBSPECTRUM_ERROR_NONE) {
        return error;
    }
                                     ...
                                     int *compressed)
{
    ...
    libspectrum_snap_set_r(snap, ((header[11] & 0x7f) + ((header[12] & 0x01) << 7)));
    ...
    if (libspectrum_snap_pc(snap) == 0) { // PC == 0x0000 => v2 or greater
        ...
        error = get_joystick_type(snap, (extra_header + 31), ((header[29] & 0xc0) >> 6), *version);
        if (error) {
            return error;
        }
        ...
        if (*version >= 3) {
            ...
            quarter_tstates = libspectrum_timings_tstates_per_frame(libspectrum_snap_machine(snap)) / 4;

            /* This is correct, even if it does disagree with Z80 v3.05's `z80dump';
               thanks to Pedro Gimeno for pointing this out when this code was part of SnapConv */
        ...
        /* I don't like this any more than you do but here is support for the Spectaculator extensions:
...
static libspectrum_error get_machine_type_extension(libspectrum_snap *snap, libspectrum_byte type)
{
    switch (type) {
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN,
                                    "%s:get_machine_type: unknown extension machine type %d",
                                    __FILE__,
                                    type);
        return LIBSPECTRUM_ERROR_UNKNOWN;
    }
                                           ...
                                           int version)
{
    ...
    libspectrum_snap_set_joystick_active_count(snap, 1);
    libspectrum_snap_set_joystick_inputs(snap,
                                         0,
                                         LIBSPECTRUM_JOYSTICK_INPUT_KEYBOARD | LIBSPECTRUM_JOYSTICK_INPUT_JOYSTICK_1);

    switch (version) {

        case 1:
        case 2:
            error = get_joystick_type_v1(snap, type);
            if (error) {
                return error;
            }
            break;
...
static libspectrum_error get_joystick_type_v1(libspectrum_snap *snap, libspectrum_byte type)
{
    switch (type) {
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN,
                                    "%s:get_joystick_type_v1: unknown v1 joystick type %d",
                                    __FILE__,
                                    type);
            return LIBSPECTRUM_ERROR_UNKNOWN;
    }
                                              ...
                                              libspectrum_byte type)
{
    switch (type) {
        ...
        case Z80_JOYSTICK_USER_DEFINED_V3:
            /* User defined mapping, check to see if keys match Sinclair Interface 2 left port
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN,
                                    "%s:get_joystick_type_v3: unknown v3 joystick type %d",
                                    __FILE__,
                                    type);
            return LIBSPECTRUM_ERROR_UNKNOWN;
        }
                                     ...
                                     int compressed)
{
    ...
    while (next_block < end) {
        ...
        /* If it looks like some .slt data, try and parse that. That should then be the end of the file */
        if (error == LIBSPECTRUM_ERROR_SLT) {
            ...
            if (next_block != end) {
                libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_blocks: .slt data does not end file");
                                  ...
                                  const libspectrum_byte *end)
{
    ...
    while (1) {
        ...
        if ((*next_block + 8) > end) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_slt: out of data in directory");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
        ...
        if (type == LIBSPECTRUM_SLT_TYPE_END) {
            break;
        }
        ...
        switch (type) {
            ...
            case LIBSPECTRUM_SLT_TYPE_LEVEL:    // Level data
                ...
                if (level >= 0x100) {
                    libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_slt: unexpected level number %d", level);
                    return LIBSPECTRUM_ERROR_CORRUPT;
                }

                // Each level should appear once only
                if (slt_length[level]) {
                    libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_slt: level %d is duplicated", level);
                    return LIBSPECTRUM_ERROR_CORRUPT;
                }

                offsets[level] = whence;
                slt_length[level] = length;
            ...
            case LIBSPECTRUM_SLT_TYPE_SCREEN: // Loading screen
                ...
                if (screen_length != 0) {
                    libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_slt: duplicated loading screen");
                    return LIBSPECTRUM_ERROR_CORRUPT;
                }

                libspectrum_snap_set_slt_screen_level(snap, level);
                screen_length = length;
                screen_offset = whence;

                break;

            default:

                libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "read_slt: unknown data type %d", type);
                return LIBSPECTRUM_ERROR_UNKNOWN;
    ...
    for (i = 0; i < 256; i++) {
        ...
        if (slt_length[i]) {
            ...
            if ((*next_block + offsets[i] + slt_length[i]) > end) {
                libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_slt: out of data reading level %d", i);
                return LIBSPECTRUM_ERROR_CORRUPT;
            }
    ...
    if (screen_length) {
        ...
        if (screen_length == 6912) { // Not compressed
            ...
            uncompress_block(&buffer, &screen_length, (*next_block + screen_offset), screen_length);

            // A screen should be 6912 bytes long
            if (screen_length != 6912) {
                libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_slt: screen is not 6912 bytes long");
                                    ...
                                    int compressed)
{
    ...
    if (version == 1) {
        ...
        if ((page <= 0) || (page > 18)) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "read_block: unknown page %d", page);
            libspectrum_free(uncompressed);
            return LIBSPECTRUM_ERROR_UNKNOWN;
        }
        ...
        // Page 11 is the Multiface ROM unless we're emulating something Scorpion-like
        ...
        if (libspectrum_snap_pages(snap, page) == NULL) {
            ...
            libspectrum_free(uncompressed);
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_block: page %d duplicated", page);
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
                                       ...
                                       const libspectrum_byte *end)
{
    if (is_compressed) {
        ...
        while (state != 4) {
            ...
            if (ptr == end) {
                libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_v1_block: end marker not found");
                return LIBSPECTRUM_ERROR_CORRUPT;
            }
            ...
            switch (state) {
                ...
                case 3:
                    ...
                    break;
                default:
                    libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "read_v1_block: unknown state %d", state);
                    return LIBSPECTRUM_ERROR_LOGIC;
            }
        ...
        uncompress_block(uncompressed, &uncompressed_length, buffer, (ptr - buffer - 4));

        // Uncompressed data must be exactly 48Kb long
        if (uncompressed_length != 0xc000) {
            libspectrum_free(*uncompressed);
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_v1_block: data does not uncompress to 48Kb");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
        ...
        *next_block = ptr;
        ...
        if ((end - *next_block) < 0xc000) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_v1_block: not enough data in buffer");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
                                       ...
                                       const libspectrum_byte *end)
{
    ...
    if (length2 != 0xffff) {
        ...
        if ((buffer + 3 + length2) > end) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_v2_block: not enough data in buffer");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
...
libspectrum_error libspectrum_z80_write2(libspectrum_buffer *buffer,
                                         int *out_flags,
                                         libspectrum_snap *snap,
                                         int in_flags)
{
    ...
    *out_flags = 0;
    ...
    // .z80 format doesn't store the 'last instruction EI', 'halted' state or the 'last instruction set Flags'
    ...
    // .z80 format may be able to store an IF2 ROM instead of the 48K ROM but not for now
    ...
    // .z80 format has partial support for writing custom ROMs but we don't use it, if you want that just use szx
    ...
    // .z80 format doesn't save the DISCiPLE state well but we don't support what is there either
...
static void write_base_header(libspectrum_buffer *buffer, int *flags, libspectrum_snap *snap)
{
    ...
    libspectrum_buffer_write_byte(buffer,
                                  ((libspectrum_snap_r(snap) >> 7) + ((libspectrum_snap_out_ula(snap) & 0x07) << 1)));
    ...
    libspectrum_buffer_write_byte(buffer, (libspectrum_snap_iff1(snap) ? 0xff : 0x00));
    libspectrum_buffer_write_byte(buffer, (libspectrum_snap_iff2(snap) ? 0xff : 0x00));
    ...
    libspectrum_buffer_write_byte(buffer,
                                  (libspectrum_snap_im(snap) & 0x03) +
                                      (libspectrum_snap_issue2(snap) ? 0x04 : 0x00) + joystick_flags);
}
...
static libspectrum_error write_extended_header(libspectrum_buffer *buffer, int *flags, libspectrum_snap *snap)
{
    ...
    switch (libspectrum_snap_machine(snap)) {
        ...
        case LIBSPECTRUM_MACHINE_PLUS2:
            ...
            machine_byte = Z80_MACHINE_PLUS2;
            break;
        ...
        case LIBSPECTRUM_MACHINE_TC2048:
            ...
            machine_byte = Z80_MACHINE_TC2048;
            break;
        case LIBSPECTRUM_MACHINE_TC2068:
            machine_byte = Z80_MACHINE_TC2068;
            break;
        case LIBSPECTRUM_MACHINE_TS2068:
            machine_byte = Z80_MACHINE_TS2068;
            break;
    ...
    header_length = second_memory_port ? LIBSPECTRUM_Z80_V3X_LENGTH : LIBSPECTRUM_Z80_V3_LENGTH;
    ...
    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_128_MEMORY) {
        libspectrum_buffer_write_byte(buffer, libspectrum_snap_out_128_memoryport(snap));
    } else if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_TIMEX_MEMORY) {
        libspectrum_buffer_write_byte(buffer, libspectrum_snap_out_scld_hsr(snap));
    } else {
        libspectrum_buffer_write_byte(buffer, '\0');
    }

    if (capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_TIMEX_VIDEO) {
        libspectrum_buffer_write_byte(buffer, libspectrum_snap_out_scld_dec(snap));
        if (libspectrum_snap_interface1_active(snap) && libspectrum_snap_interface1_paged(snap)) {
            *flags |= LIBSPECTRUM_FLAG_SNAPSHOT_MAJOR_INFO_LOSS;
        }
    } else if (libspectrum_snap_interface1_active(snap) && libspectrum_snap_interface1_paged(snap)) {
        libspectrum_buffer_write_byte(buffer, 0xff);
    } else {
        libspectrum_buffer_write_byte(buffer, '\0');
    }

    // Support 16K snapshots via Spectaculator's extension; see the comment in read_header for details
    if (libspectrum_snap_machine(snap) == LIBSPECTRUM_MACHINE_16) {
        hardware_flag |= 0x80;
    }

    // Support Fuller Box in snapshots via Spectaculator's extension; see the comment in read_header for details
    ...
    libspectrum_buffer_write_byte(buffer, libspectrum_snap_out_ay_registerport(snap));

    for (i = 0; i < 16; i++) {
        libspectrum_buffer_write_byte(buffer, libspectrum_snap_ay_registers(snap, i));
    }
    quarter_states = libspectrum_timings_tstates_per_frame(libspectrum_snap_machine(snap)) / 4;

    libspectrum_buffer_write_word(buffer, (quarter_states - (libspectrum_snap_tstates(snap) % quarter_states) - 1));

    libspectrum_buffer_write_byte(buffer, (((libspectrum_snap_tstates(snap) / quarter_states) + 3) % 4));

    // Spectator disabled
    libspectrum_buffer_write_byte(buffer, '\0');

    // MGT only for +D for now
    if (libspectrum_snap_plusd_active(snap) && libspectrum_snap_plusd_paged(snap)) {
        libspectrum_buffer_write_byte(buffer, 0xff);
    } else {
    ...
    /* Is 0x0000 to 0x3fff RAM? True if we're in a +3 64Kb RAM configuration,
       or a Scorpion configuration with page 0 mapped in.
    ...
    } else if (libspectrum_snap_plusd_active(snap) && libspectrum_snap_plusd_paged(snap)) {
        first_8kb_ram = 0;
        second_8kb_ram = 1;
    }
    ...
    // Joystick settings
    if (libspectrum_snap_joystick_list(snap, 1) == LIBSPECTRUM_JOYSTICK_SINCLAIR_2) {
        /* First 10 bytes are spectrum keyboard mappings for user defined keys
           in this order: left, right, down, up, fire */
    ...
    if (second_memory_port) {
        libspectrum_buffer_write_byte(buffer, libspectrum_snap_out_plus3_memoryport(snap));
    }
...
static void write_pages(libspectrum_buffer *buffer, libspectrum_snap *snap, int compress)
{
    ...
    if (libspectrum_snap_interface1_active(snap) && libspectrum_snap_interface1_custom_rom(snap)) {
    ...
    if (libspectrum_snap_plusd_active(snap) && libspectrum_snap_plusd_custom_rom(snap)) {
        ...
        memcpy((uncompressed + 0x2000), libspectrum_snap_plusd_ram(snap, 0), 0x2000);
    ...
    if (!(capabilities & LIBSPECTRUM_MACHINE_CAPABILITY_128_MEMORY)) {
        ...
        write_page(buffer, 4, libspectrum_snap_pages(snap, 2), compress);
        write_page(buffer, 5, libspectrum_snap_pages(snap, 0), compress);
        write_page(buffer, 8, libspectrum_snap_pages(snap, 5), compress);

    } else {

        for (i = 0; i < 8; i++) {
            if (libspectrum_snap_pages(snap, i)) {
                write_page(buffer, (i + 3), libspectrum_snap_pages(snap, i), compress);
            }
        }
    ...
    // Do we want to write .slt data? Definitely if we've got a loading screen
...
static void write_page(libspectrum_buffer *buffer, int page_num, libspectrum_byte *page, int compress)
{
    ...
    if (!compress || (!(compress & LIBSPECTRUM_FLAG_SNAPSHOT_ALWAYS_COMPRESS) && (compressed_length >= 0x4000))) {
...
static void write_slt(libspectrum_buffer *buffer, libspectrum_snap *snap)
{
    ...
    size_t compressed_screen_length;
    libspectrum_byte *compressed_screen;
    ...
    // Now write out the .slt directory, compressing the data along the way
    for (i = 0; i < 256; i++) {
        if (libspectrum_snap_slt_length(snap, i)) {

            // Zero the compressed length so it will be allocated memory by compress_block
            compressed_length[i] = 0;
            compress_block(&compressed_data[i],
                           &compressed_length[i],
                           libspectrum_snap_slt(snap, i),
                           libspectrum_snap_slt_length(snap, i));

            write_slt_entry(buffer, LIBSPECTRUM_SLT_TYPE_LEVEL, i, compressed_length[i]);
        }
    }
    ...
    if (libspectrum_snap_slt_screen(snap)) {
        ...
        compress_block(&compressed_screen, &compressed_screen_length, libspectrum_snap_slt_screen(snap), 6912);
    ...
    for (i = 0; i < 256; i++) {
        if (libspectrum_snap_slt_length(snap, i)) {
            libspectrum_buffer_write(buffer, compressed_data[i], compressed_length[i]);
        }
    }

    // And the loading screen
    if (libspectrum_snap_slt_screen(snap)) {
        libspectrum_buffer_write(buffer, compressed_screen, compressed_screen_length);
    }

    // Free up the compressed data
    if (libspectrum_snap_slt_screen(snap)) {
        libspectrum_free(compressed_screen);
    }
    for (i = 0; i < 256; i++) {
        if (libspectrum_snap_slt_length(snap, i)) {
                           ...
                           size_t src_length)
{
    ...
    while (in_ptr < (src + src_length)) {
        ...
        /* Now see if we're pointing to a run of identical bytes,
           and the last thing output wasn't a single 0xed */
        if ((*in_ptr == *(in_ptr + 1)) && !last_char_ed) {
            ...
            while ((in_ptr < (src + src_length)) && (*in_ptr == repeated) && (run_length < 0xff)) {
                run_length++;
                in_ptr++;
            }
                             ...
                             size_t src_length)
{
    ...
    while (in_ptr < (src + src_length)) {
        ...
        /* If we're pointing at two successive 0xed bytes, that's a run.
           If not, just copy the byte across */
