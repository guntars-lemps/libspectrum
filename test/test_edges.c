...
static test_edge_sequence_t complete_edges_list[] = {
...
static test_edge_sequence_t zero_tail_edges_list[] = {
...
static test_edge_sequence_t no_pilot_gdb_list[] = {
    ...
    {   0, 1, 259}, // End of block, end of tape, stop the tape (normally no edge but not at end of tape)
...
{
    return check_edges(DYNAMIC_TEST_PATH("complete-tzx.tzx"),
                       complete_edges_list,
...
{
    return check_edges(DYNAMIC_TEST_PATH("zero-tail.pzx"), zero_tail_edges_list, 0x1ff);
}


test_return_t test_29(void)
{
    return check_edges(DYNAMIC_TEST_PATH("no-pilot-gdb.tzx"), no_pilot_gdb_list, 0x1ff);
}
