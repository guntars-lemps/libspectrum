...
int read_file(libspectrum_byte **buffer, size_t *length, const char *filename)
{
    int fd;
    struct stat info;
    ssize_t bytes;
    ...
    *buffer = libspectrum_new(libspectrum_byte, *length);
    ...
    if (bytes == -1) {
        ...
        fprintf(stderr,
                "%s: read only %lu of %lu bytes from `%s'\n",
                progname,
                (unsigned long)bytes,
...
static test_return_t load_tape(libspectrum_tape **tape, const char *filename, libspectrum_error expected_result)
{
    ...
    *tape = libspectrum_tape_alloc();
    ...
    if (libspectrum_tape_read(*tape, buffer, filesize, LIBSPECTRUM_ID_UNKNOWN, filename) != expected_result) {
...
static test_return_t read_tape(const char *filename, libspectrum_error expected_result)
{
...
static test_return_t read_snap(const char *filename, const char *filename_to_pass, libspectrum_error expected_result)
{
    libspectrum_byte *buffer = NULL;
    size_t filesize = 0;
    ...
    if (libspectrum_snap_read(snap, buffer, filesize, LIBSPECTRUM_ID_UNKNOWN, filename_to_pass) != expected_result) {
        fprintf(stderr, "%s: reading `%s' did not give expected result\n", progname, filename);
...
static test_return_t play_tape(const char *filename)
{
    libspectrum_byte *buffer = NULL;
    size_t filesize = 0;
    libspectrum_tape *tape;
    ...
    if (libspectrum_tape_read(tape, buffer, filesize, LIBSPECTRUM_ID_UNKNOWN, filename)) {
        libspectrum_tape_free(tape);
        libspectrum_free(buffer);
        return TEST_INCOMPLETE;
    }
...
// Test for bugs #47 and #78: tape object incorrectly freed after reading invalid tape
static test_return_t test_1(void)
{
    return read_tape(STATIC_TEST_PATH("invalid.tzx"), LIBSPECTRUM_ERROR_UNKNOWN);
}


/* Test for bugs #84: TZX turbo blocks with zero pilot pulses and #85:
   freeing a turbo block with no data produces segfault */
static test_return_t test_2(void)
{
    libspectrum_byte *buffer = NULL;
    size_t filesize = 0;
    libspectrum_tape *tape;
    ...
    if (flags) {
        fprintf(stderr,
                "%s: reading first edge of `%s' gave unexpected flags 0x%04x; expected 0x0000\n",
                progname,
                filename,
                flags);
        libspectrum_tape_free(tape);
    ...
    if (tstates != 667) {
        fprintf(stderr,
                "%s: first edge of `%s' was %u tstates; expected 667\n",
                progname,
                filename,
                tstates);
        libspectrum_tape_free(tape);
...
static test_return_t test_3(void)
{
    libspectrum_tape *tape;
...
static test_return_t test_4(void)
{
...
static test_return_t test_5(void)
{
...
static test_return_t test_6(void)
{
...
static test_return_t test_7(void)
{
...
static test_return_t test_8(void)
{
...
static test_return_t test_9(void)
{
...
static test_return_t test_10(void)
{
...
static test_return_t test_11(void)
{
...
static test_return_t test_12(void)
{
    return read_tape(STATIC_TEST_PATH("invalid-custominfo.tzx"), LIBSPECTRUM_ERROR_CORRUPT);
}


// Test for bug #113: loop end without a loop start block accesses uninitialised memory
static test_return_t test_13(void)
{
    libspectrum_byte *buffer = NULL;
    size_t filesize = 0;
    libspectrum_tape *tape;
...
static test_return_t test_14(void)
{
...
static test_return_t test_16(void)
{
...
static test_return_t test_17(void)
{
...
static test_return_t test_18(void)
{
...
static test_return_t test_19(void)
{
    libspectrum_byte *buffer = NULL;
    size_t length = 0;
    libspectrum_tape *tape;
...
static test_return_t test_20(void)
{
...
static test_return_t test_21(void)
{
...
static test_return_t test_22(void)
{
    libspectrum_byte *buffer = NULL;
    size_t filesize = 0;
    ...
    /* writeprotected.mdr deliberately includes an extra 0 on the end;
       we want this in the buffer so we know what happens if we read off the end of the file;
       however, we don't want it in the length */
    filesize--;
...
static test_return_t test_23(void)
{
    libspectrum_byte *buffer = NULL;
...
static test_return_t test_24(void)
{
    const char *filename = DYNAMIC_TEST_PATH("complete-tzx.tzx");
    libspectrum_byte *buffer = NULL;
    size_t filesize = 0;
    ...
    while (block) {
        ...
        if (actual_size != *next_size) {
            fprintf(stderr,
                    "%s: block had length %lu, but expected %lu\n",
                    progname,
...
static test_return_t test_25(void)
{
    ...
    if (libspectrum_snap_write(&buffer, &length, &flags, snap, LIBSPECTRUM_ID_SNAPSHOT_SNA, NULL, 0) !=
    ...
    if (libspectrum_snap_read(snap, buffer, length, LIBSPECTRUM_ID_SNAPSHOT_SNA, NULL) != LIBSPECTRUM_ERROR_NONE) {
...
static test_return_t test_26(void)
{
    ...
    if (libspectrum_snap_write(&buffer, &length, &flags, snap, LIBSPECTRUM_ID_SNAPSHOT_Z80, NULL, 0) !=
        LIBSPECTRUM_ERROR_NONE) {
    ...
    if (libspectrum_snap_read(snap, buffer, length, LIBSPECTRUM_ID_SNAPSHOT_Z80, NULL) != LIBSPECTRUM_ERROR_NONE) {
...
static test_return_t test_27(void)
{
...
static test_return_t test_30(void)
{
    libspectrum_byte *buffer = NULL;
    size_t length = 0;
    libspectrum_tape *tape;
    ...
    if (libspectrum_tape_write(&buffer, &length, tape, LIBSPECTRUM_ID_TAPE_CSW)) {
...
static test_return_t test_71(void)
{
    ...
    if (libspectrum_rzx_write(&buffer, &rzx_length, rzx, LIBSPECTRUM_ID_SNAPSHOT_SZX, NULL, 1, NULL) !=
        LIBSPECTRUM_ERROR_NONE) {
...
static test_return_t test_72(void)
{
    const char *filename = DYNAMIC_TEST_PATH("complete-tzx.tzx");
    libspectrum_byte *buffer = NULL;
    size_t filesize = 0;
    ...
    // Expect to check the next block type of 19 of the 20 blocks in the test tzx
...
static void parse_test_specs(char **specs, int count)
{
    int i, j;
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0, test = tests; i < test_count; i++, test++) {
