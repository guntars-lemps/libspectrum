...
static libspectrum_error
write_file_header(libspectrum_buffer *buffer, int *out_flags, libspectrum_snap *snap);

static void
write_crtr_chunk(libspectrum_buffer *buffer, libspectrum_buffer *crtr_data, libspectrum_creator *creator);
static void
write_z80r_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);
static void
write_spcr_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);
static void
write_amxm_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);
static void
write_joy_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, int *out_flags, libspectrum_snap *snap);
static void
write_keyb_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, int *out_flags, libspectrum_snap *snap);
static void
write_ram_pages(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int compress);
static void
write_ramp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress);
static void
...
static libspectrum_error
write_rom_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, int *out_flags, libspectrum_snap *snap, int compress);
static void
write_ay_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);
static void
write_scld_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);
static void
write_b128_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static libspectrum_error
write_opus_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static libspectrum_error
write_plsd_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static libspectrum_error
write_if1_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static void
write_zxat_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);
static void
write_atrp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress);
static void
write_zxcf_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);
static void
write_cfrp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress);
static void
write_side_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap);
static void
write_drum_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);
static void
write_snet_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static void
write_snef_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static void
write_sner_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static libspectrum_error
write_pltt_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static libspectrum_error
write_mfce_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static void
write_zmmc_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);

#ifdef HAVE_ZLIB_H

static libspectrum_error
write_if2r_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap);
...
write_dock_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data,
          libspectrum_snap *snap, int exrom_dock,
                                    ...
                                    int compress);
static libspectrum_error
write_dide_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static void
write_dirp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress);
static libspectrum_error
write_dmmc_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress);
static void
write_dmrp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress);
static void
write_zxpr_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, int *out_flags, libspectrum_snap *snap);
static void
write_covx_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap);

static void
write_chunk(libspectrum_buffer *buffer, const char *id, libspectrum_buffer *block_data);

static int
compress_data(libspectrum_buffer *dest, const libspectrum_byte *src_data, size_t src_data_length, int compress);
    ...
    *buffer += sizeof(libspectrum_dword) * 8; // Skip reserved dword space
    ...
    if (expected_length) {
        ...
        if (flags & ZXSTIF1F_COMPRESSED) {
            ...
            libspectrum_error error = libspectrum_zlib_inflate(*buffer, data_length - 40, &rom_data, &uncompressed_length);
            if (error) {
                return error;
            }
...
static libspectrum_error szx_extract_roms(libspectrum_snap *snap, libspectrum_byte *rom_data, libspectrum_dword length, libspectrum_dword expected_length)
{
    int i;
    ...
    void (*setter)(libspectrum_snap*, int, libspectrum_byte*))
{
    ...
    if (*data_remaining < 4) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "%s:read_snet_memory: not enough data for length", __FILE__);
        return LIBSPECTRUM_ERROR_UNKNOWN;
    }
    ...
    *data_remaining -= data_length;
    ...
    if (compressed) {
        ...
        *buffer += data_length;
        ...
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "%s:read_snet_memory: zlib needed for decompression\n", __FILE__);
        return LIBSPECTRUM_ERROR_UNKNOWN;
...
static void write_joy_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, int *out_flags, libspectrum_snap *snap)
{
    libspectrum_dword flags;
    size_t num_joysticks = libspectrum_snap_joystick_active_count(snap);
    int i;
...
static void write_keyb_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, int *out_flags, libspectrum_snap *snap)
{
    libspectrum_dword flags;
...
static void write_zxpr_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, int *out_flags, libspectrum_snap *snap)
{
    libspectrum_word flags;
...
static libspectrum_error write_rom_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, int *out_flags, libspectrum_snap *snap, int compress)
{
...
static void write_ram_pages(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int compress)
{
...
static void write_ramp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress)
{
...
static int compress_data(libspectrum_buffer *dest, const libspectrum_byte *src_data, size_t src_data_length, int compress)
{
...
static libspectrum_error write_if1_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress)
{
    libspectrum_byte *rom_data = NULL;
...
static libspectrum_error write_opus_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress)
{
...
static libspectrum_error write_plsd_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress)
{
    libspectrum_byte *rom_data, *ram_data;
    libspectrum_buffer *rom_buffer, *ram_buffer;
    size_t disk_rom_length, disk_ram_length;
...
static void write_atrp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress)
{
...
static void write_cfrp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress)
{
...
static libspectrum_error write_if2r_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap)
{
    libspectrum_error error;
...
static libspectrum_error write_dide_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress)
{
...
static libspectrum_error write_dmmc_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress)
{
    return write_divxxx_chunk(
...
static void write_dirp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress)
{
    write_divxxx_ram_chunk(buffer, block_data, snap, page, compress, libspectrum_snap_divide_ram, ZXSTBID_DIVIDERAMPAGE);
}


static void write_dmrp_chunk(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_snap *snap, int page, int compress)
{
    write_divxxx_ram_chunk(buffer, block_data, snap, page, compress, libspectrum_snap_divmmc_ram, ZXSTBID_DIVMMCRAMPAGE);
}
...
static libspectrum_error write_mfce_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress)
{
    libspectrum_byte *ram_data;
    libspectrum_buffer *ram_buffer;
    size_t ram_length;
...
static libspectrum_error write_pltt_chunk(libspectrum_buffer *buffer, libspectrum_buffer *data, libspectrum_snap *snap, int compress)
{
    libspectrum_byte flags = 0;
