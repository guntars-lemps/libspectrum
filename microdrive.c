...
void libspectrum_microdrive_get_block(const libspectrum_microdrive *microdrive,
                                      ...
                                      libspectrum_microdrive_block *block)
{
    ...
    block->hdflag = *(d++);
    block->hdbnum = *(d++);
    block->hdblen = *d + (*(d + 1) << 8);
    d += 2;
    memcpy(block->hdbnam, d, 10);
    d += 10;
    block->hdbnam[10] = '\0';
    block->hdchks = *(d++);
    ...
    block->recflg = *(d++);
    block->recnum = *(d++);
    block->reclen = *d + (*(d + 1) << 8);
    d += 2;
    memcpy(block->recnam, d, 10);
    d += 10;
    block->recnam[10] = '\0';
    block->rechks = *(d++);

    // The data itself
    memcpy(block->data, d, 512);
     d += 512;
    block->datchk = *(d++);
...
void libspectrum_microdrive_set_block(libspectrum_microdrive *microdrive,
                                      libspectrum_byte which,
                                      libspectrum_microdrive_block *block)
{
    libspectrum_byte *d = &microdrive->data[which * LIBSPECTRUM_MICRODRIVE_BLOCK_LEN];
    // The block header
    *(d++) = block->hdflag;
    *(d++) = block->hdbnum;
    *(d++) = block->hdblen & 0xff;
    *(d++) = block->hdblen >> 8;
    memcpy(d, block->hdbnam, 10);
    d += 10;
    *(d++) = block->hdchks;
    ...
    *(d++) = block->reclen & 0xff;
    *(d++) = block->reclen >> 8;
    memcpy(d, block->recnam, 10);
    d += 10;
    *(d++) = block->rechks;

    // The data itself
    memcpy(d, block->data, 512);
    d += 512;
    *(d++) = block->datchk;
}
...
*/
    ...
    if ((*(data + 15) & 2) && (*(data + 17) == 0) && (*(data + 18) == 0)) {
