                                            ...
                                            size_t *outlength)
{
    ...
    if (*outlength) {
        ...
        *outptr = libspectrum_new(libspectrum_byte, *outlength);
        ...
        error = BZ2_bzBuffToBuffDecompress((char*)*outptr, &length2, (char*)bzptr, bzlength, 0, 0);
        if (error != BZ_OK) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "error decompressing bzip data");
        ...
        *outptr = libspectrum_new(libspectrum_byte, length);
            ...
            switch (error) {
                ...
                default:
                    libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC,
                                            "bzip2_inflate: serious error from BZ2_bzDecompressInit: %d",
                                            error);
                    libspectrum_free(*outptr);
                    return LIBSPECTRUM_ERROR_LOGIC;
                }
        ...
        stream.next_in = (char*)bzptr;
        stream.avail_in = bzlength;
        stream.next_out = (char*)*outptr;
        ...
        while (1) {
            ...
            switch (error) {
                ...
                case BZ_STREAM_END:    // Finished decompression
                    ...
                    if (error) {
                        libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC,
                                                "bzip2_inflate: error from BZ2_bzDecompressEnd: %d",
                                                error);
                        libspectrum_free(*outptr);
                        return LIBSPECTRUM_ERROR_LOGIC;
                    }
                ...
                default:
                    libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC,
                                            "bzip2_inflate: serious error from BZ2_bzDecompress: %d",
                                            error);
