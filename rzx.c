...
typedef struct libspectrum_rzx_frame_t {
    ...
    int repeat_last; // Set if we should use the last frame's IN bytes
...
typedef struct input_block_t {
    ...
    /* Used for recording to note the last non-repeated frame.
       We can't really use a direct pointer to the frame here as that will move
       around when we do a renew on the array, so just dereference it every time */
    size_t non_repeat;
...
static libspectrum_error rzx_read_header(const libspectrum_byte **ptr, const libspectrum_byte *end);
static libspectrum_error rzx_read_creator(const libspectrum_byte **ptr, const libspectrum_byte *end);
static libspectrum_error rzx_read_snapshot(libspectrum_rzx *rzx,
                                           const libspectrum_byte **ptr,
                                           const libspectrum_byte *end);
static libspectrum_error rzx_read_input(libspectrum_rzx *rzx,
                                        const libspectrum_byte **ptr,
                                        const libspectrum_byte *end);
static libspectrum_error rzx_read_frames(input_block_t *block,
                                         const libspectrum_byte **ptr,
                                         const libspectrum_byte *end);
static libspectrum_error rzx_read_sign_start(libspectrum_rzx *rzx,
                                             const libspectrum_byte **ptr,
                                             const libspectrum_byte *end);
static libspectrum_error rzx_read_sign_end(libspectrum_rzx *rzx,
                                           const libspectrum_byte **ptr,
                                           const libspectrum_byte *end);

static void rzx_write_header(libspectrum_buffer *buffer, int sign);
static void rzx_write_creator(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_creator *creator);
static libspectrum_error rzx_write_snapshot(libspectrum_buffer *buffer,
                                            libspectrum_buffer *block_data,
                                            libspectrum_snap *snap,
                                            libspectrum_id_t snap_format,
                                            libspectrum_creator *creator,
                                            int compress);
static void rzx_write_input(input_block_t *block,
                            libspectrum_buffer *buffer,
                            libspectrum_buffer *block_data,
                            int compress);
static libspectrum_error rzx_write_signed_start(libspectrum_buffer *buffer,
                                                libspectrum_buffer *block_data,
                                                libspectrum_rzx_dsa_key *key,
                                                libspectrum_creator *creator);
static libspectrum_error rzx_write_signed_end(libspectrum_buffer *buffer,
                                              libspectrum_buffer *block_data,
                                              libspectrum_rzx_dsa_key *key);
...
static void block_alloc(rzx_block_t **block, libspectrum_rzx_block_id type)
{
...
static libspectrum_error block_free(rzx_block_t *block)
{
    size_t i;
    input_block_t *input;
#ifdef HAVE_GCRYPT_H
    ...
    libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC,
                            "unknown RZX block type %d at %s:%d",
                            block->type,
                            __FILE__,
                            __LINE__);
...
static void block_free_wrapper(gpointer data, gpointer user_data)
{
    block_free(data);
}


static gint find_block(gconstpointer a, gconstpointer b)
{
...
void libspectrum_rzx_start_input(libspectrum_rzx *rzx, libspectrum_dword tstates)
{
    rzx_block_t *block;
...
libspectrum_error libspectrum_rzx_add_snap(libspectrum_rzx *rzx, libspectrum_snap *snap, int automatic)
{
    rzx_block_t *block;
...
libspectrum_error libspectrum_rzx_rollback(libspectrum_rzx *rzx, libspectrum_snap **snap)
{
    ...
    while (1) {
        list = g_slist_find_custom(list, GINT_TO_POINTER(LIBSPECTRUM_RZX_SNAPSHOT_BLOCK), find_block);
        if (!list) {
            break;
        }
    ...
    if (!previous) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "no snapshot block found in recording");
...
libspectrum_error libspectrum_rzx_rollback_to(libspectrum_rzx *rzx, libspectrum_snap **snap, size_t which)
{
...
static libspectrum_error input_block_resize(input_block_t *input, size_t new_count)
{
    libspectrum_rzx_frame_t *ptr;
    size_t new_allocated;

    // Get more space if we need it; allocate twice as much as we currently have, with a minimum of 50
    if (new_count > input->allocated) {

        new_allocated = (input->allocated >= 25) ? (2 * input->allocated) : 50;
        if (new_allocated < new_count) {
            new_allocated = new_count;
        }

        ptr = libspectrum_renew(libspectrum_rzx_frame_t, input->frames, new_allocated);
        if (!ptr) {
                                              ...
                                              libspectrum_byte *in_bytes)
{
    ...
    if ((input->count != 0) && (count != 0) && (count == input->frames[input->non_repeat].count) &&
        (!memcmp(in_bytes, input->frames[input->non_repeat].in_bytes, count))) {
        ...
        if (count) {
            ...
            memcpy(frame->in_bytes, in_bytes, (count * sizeof(*(frame->in_bytes))));
...
libspectrum_error libspectrum_rzx_start_playback(libspectrum_rzx *rzx, int which, libspectrum_snap **snap)
{
    ...
    *snap = NULL;
    ...
    for (i = which, previous = NULL, list = rzx->blocks; list; previous = list, list = list->next) {
...
libspectrum_error libspectrum_rzx_playback_frame(libspectrum_rzx *rzx, int *finished, libspectrum_snap **snap)
{
    *snap = NULL;
    *finished = 0;
    ...
    if (rzx->in_count != rzx->data_frame->count) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT,
                                "libspectrum_rzx_playback_frame: "
                                    "wrong number of INs in frame %lu: expected %lu, got %lu",
                                (unsigned long)rzx->current_frame,
                                (unsigned long)rzx->data_frame->count,
                                (unsigned long)rzx->in_count);
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    // Move the data frame pointer along, unless we're supposed to be repeating the last frame
...
libspectrum_error libspectrum_rzx_playback(libspectrum_rzx *rzx, libspectrum_byte *byte)
{
...
libspectrum_dword libspectrum_rzx_get_keyid(libspectrum_rzx *rzx)
{
    GSList *list;
    rzx_block_t *block;

    list = g_slist_find_custom(rzx->blocks, GINT_TO_POINTER(LIBSPECTRUM_RZX_SIGN_START_BLOCK), find_block);
    if (!list) {
        return 0;
    }
...
libspectrum_error libspectrum_rzx_get_signature(libspectrum_rzx *rzx, libspectrum_signature *signature)
{
    GSList *list;
    rzx_block_t *block;
    signature_block_t *sigblock;

    list = g_slist_find_custom(rzx->blocks, GINT_TO_POINTER(LIBSPECTRUM_RZX_SIGN_END_BLOCK), find_block);
    if (!list) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "no end of signed data block found");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
...
libspectrum_error libspectrum_rzx_read(libspectrum_rzx *rzx, const libspectrum_byte *buffer, size_t length)
{
    libspectrum_error error;
...
static libspectrum_error rzx_read_header(const libspectrum_byte **ptr, const libspectrum_byte *end)
{
    ...
    if ((end - (*ptr)) < ((ptrdiff_t)strlen(rzx_signature) + 6)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_header: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    // Check the RZX signature exists
    if (memcmp(*ptr, rzx_signature, strlen(rzx_signature))) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_SIGNATURE, "rzx_read_header: RZX signature not found");
    ...
    // And skip the flags as well - separate call as we'll need the return value if we ever fix the stuff below
...
static libspectrum_error rzx_read_creator(const libspectrum_byte **ptr, const libspectrum_byte *end)
{
    ...
    if ((end - (*ptr)) < 28) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_creator: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    // Check there's still enough data (the -5 is because we've already read the block ID and the length)
                                           ...
                                           const libspectrum_byte *end)
{
    ...
    int compressed;
    libspectrum_byte *gzsnap = NULL;
    size_t uncompressed_length = 0;

    if ((end - (*ptr)) < 16) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_snapshot: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    /* We don't handle 'links' to external snapshots.
       I really think these are just more trouble than they're worth */
    if (flags & 0x01) {
        /* TODO log error if this isn't the first block as that can't be easily covered
           by prompting the user for a snapshot file
           (but should be unlikely to encounter in reality?) */
        (*ptr) += blocklength - 9;
        return LIBSPECTRUM_ERROR_NONE;
    }
    ...
    if (compressed) {
        ...
        error = libspectrum_zlib_inflate(((*ptr) + 8), (blocklength - 17), &gzsnap, &uncompressed_length);
        if (error != LIBSPECTRUM_ERROR_NONE) {
            return error;
        }

        if (uncompressed_length != snaplength) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_snapshot: compressed snapshot has wrong length");
        ...
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "rzx_read_snapshot: zlib needed for decompression\n");
        return LIBSPECTRUM_ERROR_UNKNOWN;
        ...
        if (blocklength != (snaplength + 17)) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_snapshot: inconsistent snapshot lengths");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
        snap_ptr = (*ptr) + 8;
    ...
    for (done = 0, type = snapshot_strings; type->format; type++) {
        if (!strncasecmp((char*)*ptr, type->string, 4)) {
            error = libspectrum_snap_read(snap, snap_ptr, uncompressed_length, type->format, NULL);
            done = 1;
        }
    }
    ...
    if (!done) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN,
                                "%s:rzx_read_snapshot: unrecognised snapshot format",
                                __FILE__);
        if (compressed) {
            libspectrum_free(gzsnap);
        }
...
static libspectrum_error rzx_read_input(libspectrum_rzx *rzx, const libspectrum_byte **ptr, const libspectrum_byte *end)
{
    size_t blocklength;
    libspectrum_dword flags;
    int compressed;
    ...
    // Check we've got enough data for the block
    if ((end - (*ptr)) < 18) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_input: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    if (compressed) {
        ...
        const libspectrum_byte *data_ptr;
...
static libspectrum_error rzx_read_frames(input_block_t *block, const libspectrum_byte **ptr, const libspectrum_byte *end)
{
    size_t i, j;
    ...
    for (i = 0; i < block->count; i++) {
        ...
        if ((end - (*ptr)) < 4) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_frames: not enough data in buffer");
            for (j = 0; j < i; j++) {
                if (!block->frames[i].repeat_last) {
                    libspectrum_free(block->frames[j].in_bytes);
                }
            }
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
        ...
        if (block->frames[i].count) {
            ...
            block->frames[i].in_bytes = libspectrum_new(libspectrum_byte, block->frames[i].count);
                                             ...
                                             const libspectrum_byte *end)
{
    ...
    if ((end - (*ptr)) < 4) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_sign_start: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    // Check there's still enough data (the -5 is because we've already read the length and the block ID)
    if ((end - (*ptr)) < ((ptrdiff_t)length - 5)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_sign_start: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
                                           ...
                                           const libspectrum_byte *end)
{
    ...
    // Check we've got enough data for the length
    if ((end - (*ptr)) < 4) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "rzx_read_sign_end: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    // Get the length; the -5 is because we've read the block ID and the length bytes
    ...
    error = gcry_mpi_scan(&signature->r, GCRYMPI_FMT_PGP, *ptr, length, &mpi_length);
    if (error) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "error reading 'r': %s", gcry_strerror(error));
        libspectrum_free(block);
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    (*ptr) += mpi_length;
    ...
    error = gcry_mpi_scan(&signature->s, GCRYMPI_FMT_PGP, *ptr, length, &mpi_length);
    if (error) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "error reading 's': %s", gcry_strerror(error));
        gcry_mpi_release(signature->r);
        libspectrum_free(block);
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
...
libspectrum_error libspectrum_rzx_write(libspectrum_byte **buffer,
                                        size_t *length,
                                        libspectrum_rzx *rzx,
                                        libspectrum_id_t snap_format,
                                        libspectrum_creator *creator,
                                        ...
                                        libspectrum_rzx_dsa_key *key)
{
    ...
    if (creator) {
        rzx_write_creator(new_buffer, block_data, creator);
    }
    ...
    for (list = rzx->blocks; list; list = list->next) {
        ...
        switch (block->type) {
            ...
            case LIBSPECTRUM_RZX_SNAPSHOT_BLOCK:
                error = rzx_write_snapshot(new_buffer,
                                           block_data,
            ...
            case LIBSPECTRUM_RZX_INPUT_BLOCK:
                ...
                rzx_write_input(&(block->types.input), new_buffer, block_data, compress);
                break;
...
static void rzx_write_block_header(libspectrum_buffer *dest, libspectrum_buffer *src, libspectrum_byte type)
{
    size_t block_length = libspectrum_buffer_get_data_size(src) + sizeof(libspectrum_dword) + sizeof(libspectrum_byte);
...
static void rzx_write_creator(libspectrum_buffer *buffer, libspectrum_buffer *block_data, libspectrum_creator *creator)
{
    ...
    libspectrum_buffer_write(block_data, libspectrum_creator_program(creator), 20);
    ...
    if (custom_length) {
        libspectrum_buffer_write(block_data, libspectrum_creator_custom(creator), custom_length);
    }
...
static libspectrum_error rzx_compress(libspectrum_buffer *dest, libspectrum_buffer *src, int *compress)
{
#ifdef HAVE_ZLIB_H
...
#ifdef HAVE_ZLIB_H
    error = libspectrum_zlib_compress(out_data, out_length, &compressed_data, &out_length);
    if ((error != LIBSPECTRUM_ERROR_NONE) || (out_length >= libspectrum_buffer_get_data_size(src))) {
        *compress = 0;

        // We couldn't compress the data, so just copy the uncompressed data across instead
        libspectrum_buffer_write_buffer(dest, src);
                                            ...
                                            int compress)
{
    ...
    if (snap_format == LIBSPECTRUM_ID_UNKNOWN) {
        // If not given a snap format, try using .z80. If that would result in major information loss, use .szx instead
        snap_format = LIBSPECTRUM_ID_SNAPSHOT_Z80;
        error = libspectrum_snap_write_buffer(block_data, &flags, snap, snap_format, creator, 0);
        if (error) {
            goto cleanup;
        }
    ...
    if (!done) {
        ...
        libspectrum_print_error(error, "%s:rzx_write_snapshot: unexpected snap type %d", __FILE__, snap_format);
        goto cleanup;
    }
                                                ...
                                                libspectrum_creator *creator)
{
    ...
    if (!key || !key->y || (strlen(key->y) < 8)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_INVALID, "rzx_write_signed_start: invalid key");
        return LIBSPECTRUM_ERROR_INVALID;
    }

    libspectrum_buffer_write_dword(block_data, strtoul(&key->y[strlen(key->y) - 8], NULL, 16));

    // Week code
    if (creator) {
        libspectrum_buffer_write_dword(block_data, libspectrum_creator_competition_code(creator));
    } else {
        libspectrum_buffer_write_dword(block_data, 0);
    }

    rzx_write_block_header(buffer, block_data, LIBSPECTRUM_RZX_SIGN_START_BLOCK);
                                              ...
                                              libspectrum_rzx_dsa_key *key)
{
    ...
    error = libspectrum_sign_data(&signature,
                                  &sig_length,
    ...
    rzx_write_block_header(buffer, block_data, LIBSPECTRUM_RZX_SIGN_END_BLOCK);
...
void libspectrum_rzx_insert_snap(libspectrum_rzx *rzx, libspectrum_snap *snap, int where)
{
    rzx_block_t *block;
...
libspectrum_rzx_iterator libspectrum_rzx_iterator_next(libspectrum_rzx_iterator it)
{
    return it->next;
}


libspectrum_rzx_iterator libspectrum_rzx_iterator_last(libspectrum_rzx *rzx)
{
...
libspectrum_rzx_block_id libspectrum_rzx_iterator_get_type(libspectrum_rzx_iterator it)
{
    rzx_block_t *block = it->data;
...
void libspectrum_rzx_iterator_delete(libspectrum_rzx *rzx, libspectrum_rzx_iterator it)
{
    block_free(it->data);
...
libspectrum_snap *libspectrum_rzx_iterator_get_snap(libspectrum_rzx_iterator it)
{
    rzx_block_t *block = it->data;
...
int libspectrum_rzx_iterator_snap_is_automatic(libspectrum_rzx_iterator it)
{
    rzx_block_t *block = it->data;
...
static libspectrum_error input_block_merge(input_block_t *input, input_block_t *next_input)
{
    libspectrum_error error;
    ...
    memcpy(&(input->frames[input->count]), next_input->frames, (next_input->count * sizeof(libspectrum_rzx_frame_t)));
...
libspectrum_error libspectrum_rzx_finalise(libspectrum_rzx *rzx)
{
    ...
    while (list) {
        ...
        if (block->type == LIBSPECTRUM_RZX_INPUT_BLOCK) {
            ...
            if (next_block->type == LIBSPECTRUM_RZX_INPUT_BLOCK) {
                error = input_block_merge(&(block->types.input), &(next_block->types.input));
                if (error) {
                    return error;
                }
