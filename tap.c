...
static libspectrum_error write_rom(libspectrum_tape_block *block, libspectrum_buffer *buffer, libspectrum_id_t type);
static libspectrum_error write_turbo(libspectrum_tape_block *block, libspectrum_buffer *buffer, libspectrum_id_t type);
static libspectrum_error write_pure_data(libspectrum_tape_block *block, libspectrum_buffer *buffer, libspectrum_id_t type);

static libspectrum_error write_tap_block(libspectrum_buffer *buffer,
                                         libspectrum_byte *data,
                                         size_t data_length,
                                         libspectrum_id_t type);
static libspectrum_error skip_block(libspectrum_tape_block *block, const char *message);
                                    ...
                                    libspectrum_id_t type)
{
    ...
    while (ptr < end) {
        ...
        // If we've got less than two bytes for the length, something's gone wrong, so gone home
        if ((end - ptr) < 2) {
            libspectrum_tape_clear(tape);
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_tap_read: not enough data in buffer");
        ...
        if ((type == LIBSPECTRUM_ID_TAPE_SPC) || (type == LIBSPECTRUM_ID_TAPE_STA) || (type == LIBSPECTRUM_ID_TAPE_LTP)) {
            data_length += 2;
        }
...
static libspectrum_error write_pure_data(libspectrum_tape_block *block,
                                         ...
                                         libspectrum_id_t type)
{
    ...
    libspectrum_print_error(LIBSPECTRUM_ERROR_WARNING,
                            "write_pure_data: converting Pure Data Block (ID 0x14); "
                                "conversion almost certainly won't work");

    error = write_tap_block(buffer, libspectrum_tape_block_data(block), libspectrum_tape_block_data_length(block), type);
    if (error != LIBSPECTRUM_ERROR_NONE) {
        return error;
    }
                                         ...
                                         libspectrum_id_t type)
{
    ...
    if ((type == LIBSPECTRUM_ID_TAPE_SPC) || (type == LIBSPECTRUM_ID_TAPE_STA) || (type == LIBSPECTRUM_ID_TAPE_LTP)) {
        if (data_length < 2) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_INVALID, "write_tap_block: block too short");
