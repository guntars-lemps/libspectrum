...
// Application specific commands, only some implemented but needed for command identification
...
libspectrum_mmc_card *libspectrum_mmc_alloc(void)
{
...
void libspectrum_mmc_free(libspectrum_mmc_card *card)
{
...
libspectrum_error libspectrum_mmc_insert(libspectrum_mmc_card *card, const char *filename)
{
    ...
    card->total_sectors = (libspectrum_dword)card->drive.cylinders * card->drive.heads * card->drive.sectors;

    if ((card->drive.sector_size != 512) || (card->total_sectors % 1024)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "Image size not supported");
        return LIBSPECTRUM_ERROR_UNKNOWN;
    }

    /* memory capacity = (C_SIZE + 1) * 512K bytes
       we reduce the sector count by 1024.
       This gives us a minimum card size of 512 Kb. Not too worried about that. */
    c_size = (card->total_sectors >> 10) - 1;

    // We emulate a SDHC card which has a maximum size of (32 Gb - 80 Mb)
    if (c_size >= 65375) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "Image size too big (>32 Gb)");
        return LIBSPECTRUM_ERROR_UNKNOWN;
    }
...
void libspectrum_mmc_eject(libspectrum_mmc_card *card)
{
...
void libspectrum_mmc_reset(libspectrum_mmc_card *card)
{
...
int libspectrum_mmc_dirty(libspectrum_mmc_card *card)
{
    return g_hash_table_size(card->cache) != 0;
}


void libspectrum_mmc_commit(libspectrum_mmc_card *card)
{
    libspectrum_ide_commit_drive(&card->drive, card->cache);
}


libspectrum_byte libspectrum_mmc_read(libspectrum_mmc_card *card)
{
    libspectrum_byte r = (card->response_buffer_next < card->response_buffer_end) ?
                         *(card->response_buffer_next)++ : 0xff;

    return r;
}


static int parse_command(libspectrum_mmc_card *card, libspectrum_byte b)
{
    // All commands should have start bit == 0 and transmitter bit == 1
    if ((b & 0xc0) != 0x40) {
        return 0; // don't change current state
    }

    /* Unlike the SD Memory Card protocol, in the SPI mode, the card will always respond to a command.
       The response indicates acceptance or rejection of the command. Now wait for arguments and CRC */
    card->current_command = b & 0x3f;
    return 1;
}


static void set_response_buffer_r1(libspectrum_mmc_card *card)
{
...
static void set_response_buffer_r7(libspectrum_mmc_card *card, libspectrum_dword value)
{
    card->response_buffer[0] = card->r1_status;
...
static void reset_erase_sequence(libspectrum_mmc_card *card)
{
    card->erase_sequence = SEQ_ERASE_NONE;
    card->erase_block_start = 0;
    card->erase_block_end = 0;
...
static void erase_wr_blk_start(libspectrum_mmc_card *card)
{
    // Card initialised?
    ...
    card->erase_block_start = card->current_argument[3] + (card->current_argument[2] << 8) +
                              (card->current_argument[1] << 16) + (card->current_argument[0] << 24);
...
static void erase_wr_blk_end(libspectrum_mmc_card *card)
{
    // Card initialised?
    if (card->r1_status & IN_IDLE_STATE_MASK) {
        card->r1_status |= ILLEGAL_COMMAND_MASK;
    ...
    card->erase_block_end = card->current_argument[3] + (card->current_argument[2] << 8) +
                            (card->current_argument[1] << 16) + (card->current_argument[0] << 24);
...
static void erase(libspectrum_mmc_card *card)
{
    libspectrum_dword i;
...
static void read_single_block(libspectrum_mmc_card *card)
{
    ...
    sector_number = card->current_argument[3] + (card->current_argument[2] << 8) +
                    (card->current_argument[1] << 16) + (card->current_argument[0] << 24);
    ...
    int error = libspectrum_ide_read_sector_from_hdf(&card->drive, card->cache, sector_number, &card->response_buffer[2]);
...
static void send_csd(libspectrum_mmc_card *card)
{
    // Card initialised?
    if (card->r1_status & IN_IDLE_STATE_MASK) {
        card->r1_status |= ILLEGAL_COMMAND_MASK;
    ...
    // C_SIZE (spread 6 bits, 8 bits, 8 bits across three bytes), first 6 bits set to zero
...
static void send_cid(libspectrum_mmc_card *card)
{
    // Card initialised?
    if (card->r1_status & IN_IDLE_STATE_MASK) {
        card->r1_status |= ILLEGAL_COMMAND_MASK;
...
static int do_application_command(libspectrum_mmc_card *card)
{
    int is_acmd = 1;
...
static void do_standard_command(libspectrum_mmc_card *card)
{
    switch (card->current_command) {
        ...
        case APP_CMD:
            ...
            break;
        case READ_OCR:
            /* We set only the card capacity status (CCS, bit 30) and
               card power up status bits (bit 31). CCS set indicates an SDHC card. */
        ...
        default:
            ...
            libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "Unknown MMC command %d received", card->current_command);
            break;
    }
}


static void do_command(libspectrum_mmc_card *card)
{
    /* Previous APP_CMD indicates to the card that the next command is an application specific command
       rather than a standard command */
    if (card->cmd55_issued) {
        card->cmd55_issued = 0;

        // If a non-ACMD is sent, then it is respected by the card as a normal SD Memory Card command
    ...
    if (card->erase_sequence != SEQ_ERASE_NONE) {
        switch (card->current_command) {
            ...
            case ERASE:
                break;
            default:
                // Clear erase sequence before executing because an out of erase sequence command was received
                reset_erase_sequence(card);
...
static void write_single_block(libspectrum_mmc_card *card)
{
    libspectrum_dword sector_number;
...
void libspectrum_mmc_write(libspectrum_mmc_card *card, libspectrum_byte data)
{
    ...
    switch (card->command_state) {
        ...
        case WAITING_FOR_CRC:
            ...
            card->command_state = (card->current_command == WRITE_BLOCK) ? WAITING_FOR_DATA_TOKEN : WAITING_FOR_COMMAND;
            break;
