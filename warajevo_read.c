...
// The Warajevo .tap file signature (3rd group of 4 bytes) also end of tap marker
...
static libspectrum_error exec_command(libspectrum_byte *dest,
...
static libspectrum_error get_next_block(size_t *offset,
                                        const libspectrum_byte *buffer,
                                        const libspectrum_byte *end,
                                        libspectrum_tape *tape);

static libspectrum_error read_rom_block(libspectrum_tape *tape,
                                        const libspectrum_byte *ptr,
                                        const libspectrum_byte *end,
                                        size_t offset);

static libspectrum_error read_raw_data(libspectrum_tape *tape,
                                       const libspectrum_byte *ptr,
                                       const libspectrum_byte *end,
                                       size_t offset);

static libspectrum_error add_bit_to_copy_command(libspectrum_byte *dest,
                                                 const libspectrum_byte *src,
                                                 const libspectrum_byte *end,
...
static void reset_copy_command(void);

static libspectrum_error decompress_block(libspectrum_byte *dest,
                                          const libspectrum_byte *src,
                                          const libspectrum_byte *end,
                                          size_t signature,
                                          size_t length);
...
libspectrum_error internal_warajevo_read(libspectrum_tape *tape, const libspectrum_byte *buffer, size_t length)
{
    ...
    // Must be at least as many bytes as the signature, the initial block pointer and source pointer block
    if (length < 12) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_warajevo_read: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    // Now check the signature
    if (lsb2dword(ptr + 8) != warajevo_signature) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_SIGNATURE, "libspectrum_warajevo_read: wrong signature");
...
static libspectrum_error execute_copy_command(libspectrum_byte *dest, const libspectrum_byte *end, size_t *bytes_written)
{
    if (((*bytes_written + 1) < command.offset) ||
        ((dest + *bytes_written - command.offset + 1 + command.size) > end)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "execute_copy_command: corrupt compressed block in file");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    memcpy((dest + *bytes_written), (dest + *bytes_written - command.offset + 1), command.size);
...
static libspectrum_error read_rom_block(libspectrum_tape *tape,
                                        ...
                                        size_t offset)
{
    libspectrum_tape_block *block = libspectrum_tape_block_alloc(LIBSPECTRUM_TAPE_BLOCK_ROM);
    libspectrum_error error;
    ...
    if ((end - data) < (ptrdiff_t)block_size) {
        ...
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "warajevo_read_rom_block: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    if (size == COMPRESSED_BLOCK) {
        ...
        error = decompress_block((block_data + 1), data, end, lsb2word(ptr + offset + 15), (length - 2));
        if (error) {
            libspectrum_free(block_data);
            libspectrum_free(block);
            return error;
        }
    } else {
...
static libspectrum_error read_raw_data(libspectrum_tape *tape,
                                       ...
                                       size_t offset)
{
    libspectrum_tape_block *block = libspectrum_tape_block_alloc(LIBSPECTRUM_TAPE_BLOCK_RAW_DATA);
    ...
    status_type status;
    size_t length, bit_length;
    libspectrum_byte *block_data;
    ...
    if ((end - data) < (ptrdiff_t)compressed_size) {
        libspectrum_free(block);
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "warajevo_read_raw_data: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    if (compressed_size != decompressed_size) {
        ...
        error = decompress_block(block_data, data, end, lsb2word(ptr + offset + 15), length);
        if (error) {
            libspectrum_free(block_data);
            libspectrum_free(block);
            return error;
    ...
    switch (status.bits.frequency) {
        ...
        case HZ44100:
            ...
            break;
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "read_raw_data: unknown frequency %d", status.bits.frequency);
            libspectrum_free(block_data);
            libspectrum_free(block);
    ...
    libspectrum_tape_block_set_bits_in_last_byte(block, (status.bits.bits_used + 1));
    ...
    /* Warajevo TAPs have a relatively short limit on RAW blocks of 64995 bytes
       ...
       coalesing adjacent compatible RAW blocks to ensure the pulse structure is preserved. */
    ...
    if (last_block && (libspectrum_tape_block_type(last_block) == LIBSPECTRUM_TAPE_BLOCK_RAW_DATA) &&
        (libspectrum_tape_block_bit_length(last_block) == bit_length) &&
        (libspectrum_tape_block_bits_in_last_byte(last_block) == 8)) {
        // Combine the two blocks
        size_t new_length = libspectrum_tape_block_data_length(last_block) + length;
        block_data = libspectrum_renew(libspectrum_byte, libspectrum_tape_block_data(last_block), new_length);

        memcpy((block_data + libspectrum_tape_block_data_length(last_block)), libspectrum_tape_block_data(block), length);
        libspectrum_tape_block_set_data(last_block, block_data);
        libspectrum_tape_block_set_data_length(last_block, new_length);
        libspectrum_tape_block_set_bits_in_last_byte(last_block, (status.bits.bits_used + 1));
