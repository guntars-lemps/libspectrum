...
/* Operations on identity fields.
   For reasons best known to Ramsoft, these (together with the disk data itself) are stored in
   Intel little-endian format rather than the way the ATA spec describes.
*/
...
static gboolean write_to_disk(gpointer key, gpointer value, gpointer user_data);
static gboolean clear_cache(gpointer key, gpointer value, gpointer user_data GCC_UNUSED);
...
static void write_data(libspectrum_ide_channel *chn, libspectrum_byte data);
...
static void execute_command(libspectrum_ide_channel *chn, libspectrum_byte data);


// Initialise a libspectrum_ide_channel structure
libspectrum_ide_channel* libspectrum_ide_alloc(libspectrum_ide_databus databus)
{
    ...
    channel->cache[LIBSPECTRUM_IDE_MASTER] = g_hash_table_new(g_int_hash, g_int_equal);
    channel->cache[LIBSPECTRUM_IDE_SLAVE] = g_hash_table_new(g_int_hash, g_int_equal);
...
libspectrum_error libspectrum_ide_free(libspectrum_ide_channel *chn)
{
...
libspectrum_error libspectrum_ide_insert_into_drive(libspectrum_ide_drive *drv, const char *filename)
{
    FILE *f;
    size_t l;
    ...
    if (!f) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN,
                                "libspectrum_ide_insert: unable to open file '%s': %s",
                                filename,
                                strerror(errno));
    ...
    if (l != sizeof(libspectrum_hdf_header)) {
        fclose(f);
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN,
                                "libspectrum_ide_insert: unable to read HDF header from '%s'",
                                filename);
        return LIBSPECTRUM_ERROR_UNKNOWN;
    }

    // Verify the validity of the header
    if (memcmp(&drv->hdf.signature, "RS-IDE", 6) || (drv->hdf.id != 0x1a)) {
        fclose(f);
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT,
                                "libspectrum_ide_insert: '%s' is not a valid HDF file",
                                filename);
    ...
    drv->disk = f;
    drv->data_offset = (drv->hdf.datastart_hi << 8) | (drv->hdf.datastart_low);
    drv->sector_size = (drv->hdf.flags & 0x01) ? 256 : 512;

    // Extract drive geometry from the drive identity command
    drv->cylinders = GET_WORD(drv->hdf.drive_identity, LIBSPECTRUM_IDE_IDENTITY_NUM_CYLINDERS);
    drv->heads = GET_WORD(drv->hdf.drive_identity, LIBSPECTRUM_IDE_IDENTITY_NUM_HEADS);
    drv->sectors = GET_WORD(drv->hdf.drive_identity, LIBSPECTRUM_IDE_IDENTITY_NUM_SECTORS);
...
libspectrum_error libspectrum_ide_insert(libspectrum_ide_channel *chn, libspectrum_ide_unit unit, const char *filename)
{
...
void libspectrum_ide_commit_drive(libspectrum_ide_drive *drv, GHashTable *cache)
{
    if (!drv->disk) {
        return;
    }
...
libspectrum_error libspectrum_ide_commit(libspectrum_ide_channel *chn, libspectrum_ide_unit unit)
{
...
int libspectrum_ide_dirty(libspectrum_ide_channel *chn, libspectrum_ide_unit unit)
{
...
libspectrum_error libspectrum_ide_eject(libspectrum_ide_channel *chn, libspectrum_ide_unit unit)
{
    return libspectrum_ide_eject_from_drive(&chn->drive[unit], chn->cache[unit]);
}


// Reset an IDE channel
libspectrum_error libspectrum_ide_reset(libspectrum_ide_channel *chn)
{
    // Reset channel status
    chn->selected = LIBSPECTRUM_IDE_MASTER;
    chn->phase = LIBSPECTRUM_IDE_PHASE_READY;

    if (chn->drive[LIBSPECTRUM_IDE_MASTER].disk || chn->drive[LIBSPECTRUM_IDE_SLAVE].disk) {
...
int libspectrum_ide_read_sector_from_hdf(libspectrum_ide_drive *drv,
                                         GHashTable *cache,
                                         ...
                                         libspectrum_byte *dest)
{
    ...
    if (!buffer) {
        ...
        if (fseek(drv->disk, sector_position, SEEK_SET)) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_WARNING, "Couldn't seek in HDF file\n");
            return 1;
        }

        // Read the packed data into a temporary buffer
        if (fread(packed_buf, 1, drv->sector_size, drv->disk) != drv->sector_size) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_WARNING, "Couldn't read from HDF file\n");
            return 1;
        }
...
static int read_hdf(libspectrum_ide_channel *chn)
{
    return libspectrum_ide_read_sector_from_hdf(&chn->drive[chn->selected],
                                                chn->cache[chn->selected],
                                                chn->sector_number,
                                                chn->buffer);
}


void libspectrum_ide_write_sector_to_hdf(libspectrum_ide_drive *drv,
                                         GHashTable *cache,
                                         libspectrum_dword sector_number,
...
static int write_hdf(libspectrum_ide_channel *chn)
{
    libspectrum_ide_write_sector_to_hdf(&chn->drive[chn->selected],
                                        chn->cache[chn->selected],
                                        chn->sector_number,
                                        chn->buffer);
    return 0;
...
libspectrum_byte libspectrum_ide_read(libspectrum_ide_channel *chn, libspectrum_ide_register reg)
{
    libspectrum_ide_drive *drv = &chn->drive[chn->selected];

    switch (reg) {

        case LIBSPECTRUM_IDE_REGISTER_DATA:
            return read_data(chn);
        case LIBSPECTRUM_IDE_REGISTER_DATA2:
            return chn->data2;
        case LIBSPECTRUM_IDE_REGISTER_ERROR_FEATURE:
            return drv->error;
        case LIBSPECTRUM_IDE_REGISTER_SECTOR_COUNT:
            return chn->sector_count;
        case LIBSPECTRUM_IDE_REGISTER_SECTOR:
            return chn->sector;
        case LIBSPECTRUM_IDE_REGISTER_CYLINDER_LOW:
            return chn->cylinder_low;
        case LIBSPECTRUM_IDE_REGISTER_CYLINDER_HIGH:
            return chn->cylinder_high;
...
static void write_data(libspectrum_ide_channel *chn, libspectrum_byte data)
{
    ...
    if (chn->phase != LIBSPECTRUM_IDE_PHASE_PIO_OUT) {
        return;
    }
...
static libspectrum_error seek(libspectrum_ide_channel *chn)
{
    ...
    if (chn->head & LIBSPECTRUM_IDE_HEAD_LBA) {
        ...
        sectornumber = (chn->head & (LIBSPECTRUM_IDE_HEAD_HEAD << 24)) + (chn->cylinder_high << 16) +
                       (chn->cylinder_low << 8) + (chn->sector);
        ...
        if ((cylinder >= drv->cylinders) || (head >= drv->heads) || (sector < 0) || (sector >= drv->sectors)) {
            sectornumber = -1;
        } else {
            sectornumber = (((cylinder * drv->heads) + head) * drv->sectors) + sector;
        }

    }

    // Seek to the correct position
    if ((sectornumber < 0) || (sectornumber >= (drv->cylinders * drv->heads * drv->sectors))) {
        drv->status |= LIBSPECTRUM_IDE_STATUS_ERR;
    ...
    if (chn->head & LIBSPECTRUM_IDE_HEAD_LBA) {
        ...
        if (!chn->sector) {
            ...
            if (!chn->cylinder_low) {
                ...
                if (!chn->cylinder_high) {
                    next_head = ((chn->head & LIBSPECTRUM_IDE_HEAD_HEAD) + 1) & LIBSPECTRUM_IDE_HEAD_HEAD;
        ...
        if (chn->sector == 1) {
            next_head = ((chn->head & LIBSPECTRUM_IDE_HEAD_HEAD) + 1) % drv->heads;
            chn->head = (chn->head & ~LIBSPECTRUM_IDE_HEAD_HEAD) | next_head;
            if (!next_head) {
                chn->cylinder_low = (chn->cylinder_low + 1) & 0xff;
                if (!chn->cylinder_low) {
...
static void identifydevice(libspectrum_ide_channel *chn)
{
    ...
    SET_WORD(chn->buffer, LIBSPECTRUM_IDE_IDENTITY_CURRENT_CYLINDERS, drv->cylinders);
    // Number of current logical heads
    SET_WORD(chn->buffer, LIBSPECTRUM_IDE_IDENTITY_CURRENT_HEADS, drv->heads);
    // Number of current logical sectors per logical track
    SET_WORD(chn->buffer, LIBSPECTRUM_IDE_IDENTITY_CURRENT_SECTORS, drv->sectors);
    // Current capacity in sectors
    SET_WORD(chn->buffer, LIBSPECTRUM_IDE_IDENTITY_CURRENT_CAPACITY_LOW, (sector_count & 0x0000ffff));
    SET_WORD(chn->buffer, LIBSPECTRUM_IDE_IDENTITY_CURRENT_CAPACITY_HI, (sector_count & 0xffff0000) >> 16);

    // Total number of user addressable sectors; only defined if LBA supported
    if (GET_WORD(chn->buffer, LIBSPECTRUM_IDE_IDENTITY_CAPABILITIES) & 0x0200) {
        SET_WORD(chn->buffer, LIBSPECTRUM_IDE_IDENTITY_TOTAL_SECTORS_LOW, (sector_count & 0x0000ffff));
        SET_WORD(chn->buffer, LIBSPECTRUM_IDE_IDENTITY_TOTAL_SECTORS_HI, (sector_count & 0xffff0000) >> 16);
    }

    // prevent read_data from trying to read from disk after identity block is completely read in
    chn->sector_count = 0;
...
static void execute_command(libspectrum_ide_channel *chn, libspectrum_byte data)
{
    ...
    switch (data) {
        ...
        case LIBSPECTRUM_IDE_COMMAND_READ_SECTOR_RETRY:
            readsector(chn);
            break;
        case LIBSPECTRUM_IDE_COMMAND_READ_SECTOR_NORETRY:
            readsector(chn);
            break;
        case LIBSPECTRUM_IDE_COMMAND_WRITE_SECTOR_RETRY:
            writesector(chn);
            break;
        case LIBSPECTRUM_IDE_COMMAND_WRITE_SECTOR_NORETRY:
            writesector(chn);
            break;
        case LIBSPECTRUM_IDE_COMMAND_IDENTIFY_DRIVE_ATA:
            identifydevice(chn);
            break;
        case LIBSPECTRUM_IDE_COMMAND_IDENTIFY_DRIVE_ATAPI:
            identifydevice(chn);
            break;
        case LIBSPECTRUM_IDE_COMMAND_INITIALIZE_DEVICE_PARAMETERS:
            init_device_params(chn);
            break;
...
void libspectrum_ide_write(libspectrum_ide_channel *chn, libspectrum_ide_register reg, libspectrum_byte data)
{
    switch (reg) {
        case LIBSPECTRUM_IDE_REGISTER_DATA:
            write_data(chn, data);
        ...
        case LIBSPECTRUM_IDE_REGISTER_ERROR_FEATURE:
            ...
            break;
        case LIBSPECTRUM_IDE_REGISTER_SECTOR_COUNT:
            chn->sector_count = data;
            break;
        case LIBSPECTRUM_IDE_REGISTER_SECTOR:
            chn->sector = data;
            break;
        case LIBSPECTRUM_IDE_REGISTER_CYLINDER_LOW:
            chn->cylinder_low = data;
            break;
        case LIBSPECTRUM_IDE_REGISTER_CYLINDER_HIGH:
            chn->cylinder_high = data;
            break;

        case LIBSPECTRUM_IDE_REGISTER_HEAD_DRIVE:
            chn->head = data;
            chn->selected = chn->head & LIBSPECTRUM_IDE_HEAD_DEV ? LIBSPECTRUM_IDE_SLAVE : LIBSPECTRUM_IDE_MASTER;
            break;

        case LIBSPECTRUM_IDE_REGISTER_COMMAND_STATUS:
            execute_command(chn, data);
             break;
    }
}
