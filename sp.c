...
libspectrum_error libspectrum_sp_read(libspectrum_snap *snap, const libspectrum_byte *buffer, size_t length)
{
    ...
    if (length < SP_HEADER_LENGTH) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_sp_read: not enough bytes for .sp header");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    // Check the signature
    if ((buffer[0] != 'S') || (buffer[1] != 'P')) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_SIGNATURE, "libspectrum_sp_read: 'SP' signature not present");
    ...
    if ((start + (libspectrum_dword)memory_length) > 0x10000) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_sp_read: memory dump extends beyond 0xffff");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    libspectrum_snap_set_im(snap, ((flags & 0x08) ? 0 : ((flags & 0x02) ? 2 : 1)));

    // Get me 48K of zero-ed memory and then copy in the bits that were represented in the snap
