...
static void libspectrum_dck_block_alloc(libspectrum_dck_block **dck)
{
    size_t i;
...
static libspectrum_error libspectrum_dck_block_free(libspectrum_dck_block *dck, int keep_pages)
{
    size_t i;
...
libspectrum_error libspectrum_dck_read(libspectrum_dck *dck, const libspectrum_byte *buffer, const size_t length)
{
    return libspectrum_dck_read2(dck, buffer, length, NULL);
}


libspectrum_error libspectrum_dck_read2(libspectrum_dck *dck,
                                        const libspectrum_byte *buffer,
                                        const size_t clength,
                                        const char *filename)
{
    int i;
    int num_dck_block = 0;
    ...
    // Ugly but necessary to get around the fact that 'clength' is a const parameter, but we now modify it
    length = clength;

    // For storing the uncompressed data in if the input file was compressed
    new_buffer = NULL;
    ...
    while (buffer < end) {
        ...
        if ((buffer + 9) > end) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_dck_read: not enough data in buffer");
        ...
        switch (buffer[0]) {
            ...
            case LIBSPECTRUM_DCK_BANK_HOME:
                break;
            default:
                libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "libspectrum_dck_read: unknown bank ID %d", buffer[0]);
        ...
        // Copy the page types
        for (i = 0; i < 8; i++) {
            dck->dck[num_dck_block]->access[i] = *buffer++;
        }
        ...
        for (i = 0; i < 8; i++) {
            switch (dck->dck[num_dck_block]->access[i]) {
                case LIBSPECTRUM_DCK_PAGE_NULL:
                    break;
                case LIBSPECTRUM_DCK_PAGE_RAM_EMPTY:
                    dck->dck[num_dck_block]->pages[i] = libspectrum_new0(libspectrum_byte, DCK_PAGE_SIZE);
                    if (!dck->dck[num_dck_block]->pages[i]) {
                        libspectrum_print_error(LIBSPECTRUM_ERROR_MEMORY, "libspectrum_dck_read: out of memory");
                ...
                case LIBSPECTRUM_DCK_PAGE_ROM:
                case LIBSPECTRUM_DCK_PAGE_RAM:
                    dck->dck[num_dck_block]->pages[i] = libspectrum_new(libspectrum_byte, DCK_PAGE_SIZE);
        ...
        if (num_dck_block == 256) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_MEMORY, "libspectrum_dck_read: more than 256 banks");
            error = LIBSPECTRUM_ERROR_MEMORY;
            goto end;
        }
    }
