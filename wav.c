...
libspectrum_error libspectrum_wav_read(libspectrum_tape *tape, const char *filename)
{
    ...
    if (handle == AF_NULL_FILEHANDLE) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC,
                                "libspectrum_wav_read: audiofile failed to open file:%s",
                                filename);
        return LIBSPECTRUM_ERROR_LOGIC;
    }
    ...
    tape_length = length;
    if (tape_length%8) {
        tape_length += 8 - (tape_length % 8);
    }

    buffer = libspectrum_new0(libspectrum_byte, (tape_length * afGetChannels(handle, track)));
    ...
    libspectrum_tape_block_set_bit_length(block, (3500000 / afGetRate(handle, track)));
    ...
    if (afCloseFile(handle)) {
        libspectrum_free(buffer);
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "libspectrum_wav_read: failed to close audio file");
