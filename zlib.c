...
static libspectrum_error zlib_inflate(const libspectrum_byte *gzptr,
                                      size_t gzlength,
...
*/
libspectrum_error libspectrum_zlib_inflate(const libspectrum_byte *gzptr,
                                           size_t gzlength,
                                           libspectrum_byte **outptr,
                                           size_t *outlength)
{
                                           ...
                                           size_t *outlength)
{
    ...
    error = skip_gzip_header(&gzptr, &gzlength);
    if (error) {
        return error;
    }
                                      ...
                                      int gzip_hack)
{
    ...
    *outptr = libspectrum_renew(libspectrum_byte, *outptr, *outlength);
    ...
    switch (error) {
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "gzip error from inflate: %s", stream.msg);
            libspectrum_free(*outptr);
            inflateEnd(&stream);
...
static libspectrum_error skip_gzip_header(const libspectrum_byte **gzptr, size_t *gzlength)
{
    ...
    if ((*gzptr)[2] != 8) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "unknown gzip compression method %d", (*gzptr)[2]);
        return LIBSPECTRUM_ERROR_UNKNOWN;
    }
    ...
    *gzlength -= 10;
    ...
    if (flags & 0x04) { // extra header present
        ...
        if (*gzlength < 2) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "not enough data for gzip extra header length");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
...
/* Deflates a block of data.
   ...
   Returns: error flag (libspectrum_error)
*/
libspectrum_error libspectrum_zlib_compress(const libspectrum_byte *data,
                                            size_t length,
                                            libspectrum_byte **gzptr,
                                            size_t *gzlength)
{
    ...
    *gzptr = libspectrum_new(libspectrum_byte, gzl);
    ...
    switch (gzret) {
        ...
        case Z_BUF_ERROR: // Not enough space in output buffer. Shouldn't happen
            libspectrum_free(*gzptr);
            *gzptr = 0;
