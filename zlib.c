...
static libspectrum_error
skip_null_terminated_string(const libspectrum_byte **ptr, size_t *length, const char *name);
static libspectrum_error
zlib_inflate(const libspectrum_byte *gzptr, size_t gzlength, libspectrum_byte **outptr, size_t *outlength, int gzip_hack);


libspectrum_error libspectrum_zlib_inflate(const libspectrum_byte *gzptr, size_t gzlength, libspectrum_byte **outptr, size_t *outlength)
...
libspectrum_error libspectrum_gzip_inflate(const libspectrum_byte *gzptr, size_t gzlength, libspectrum_byte **outptr, size_t *outlength)
{
    int error;
...
libspectrum_error libspectrum_zip_inflate(const libspectrum_byte *zipptr, size_t ziplength, libspectrum_byte **outptr, size_t *outlength)
{
    return zlib_inflate(zipptr, ziplength, outptr, outlength, 1);
}


static libspectrum_error zlib_inflate(const libspectrum_byte *gzptr, size_t gzlength, libspectrum_byte **outptr, size_t *outlength, int gzip_hack)
{
    z_stream stream;
    int error;
    ...
    default:
        libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "error from inflateInit2: %s", stream.msg);
        inflateEnd(&stream);
        return LIBSPECTRUM_ERROR_MEMORY;
    ...
    *outptr = libspectrum_renew(libspectrum_byte, *outptr, *outlength);
    ...
    case Z_NEED_DICT:
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "gzip inflation needs dictionary");
    ...
    case Z_BUF_ERROR:
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "not enough space in gzip output buffer");
        libspectrum_free(*outptr);
        inflateEnd(&stream);
        return LIBSPECTRUM_ERROR_CORRUPT;
    ...
    if (error != Z_OK) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "gzip error from inflateEnd: %s", stream.msg);
        libspectrum_free(*outptr);
        inflateEnd(&stream);
        return LIBSPECTRUM_ERROR_LOGIC;
    }
...
static libspectrum_error skip_gzip_header(const libspectrum_byte **gzptr, size_t *gzlength)
{
    libspectrum_byte flags;
    libspectrum_error error;

    if (*gzlength < 10) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "not enough data for gzip header");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    if ((*gzptr)[0] != 0x1f || (*gzptr)[1] != 0x8b) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "gzip header missing");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    "not enough data for gzip extra header length"
        ...
        if (*gzlength < length) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "not enough data for gzip extra header");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
    ...
    if (flags & 0x02) {        // header CRC present
        ...
        if (*gzlength < 2) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "not enough data for gzip header CRC");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
...
libspectrum_error libspectrum_zlib_compress(const libspectrum_byte *data, size_t length, libspectrum_byte **gzptr, size_t *gzlength)
  ...
  */
{
    ...
    *gzptr = libspectrum_new(libspectrum_byte, gzl);
    ...
    case Z_MEM_ERROR:        // out of memory
        ...
        libspectrum_print_error(LIBSPECTRUM_ERROR_MEMORY, "libspectrum_zlib_compress: out of memory");
        return LIBSPECTRUM_ERROR_MEMORY;

    case Z_VERSION_ERROR:        // unrecognised version
        libspectrum_free(*gzptr); *gzptr = 0;
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "libspectrum_zlib_compress: unknown version");
        return LIBSPECTRUM_ERROR_UNKNOWN;

    case Z_BUF_ERROR:        /* Not enough space in output buffer.
                   Shouldn't happen */
        libspectrum_free(*gzptr); *gzptr = 0;
        libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "libspectrum_zlib_compress: out of space?");
        return LIBSPECTRUM_ERROR_LOGIC;

    default:            // some other error
        libspectrum_free(*gzptr); *gzptr = 0;
        libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "libspectrum_zlib_compress: unexpected error?");
        return LIBSPECTRUM_ERROR_LOGIC;
    }
}
