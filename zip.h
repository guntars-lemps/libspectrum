...
typedef struct zip_local_header {
    libspectrum_dword magic;             // local file header signature 4 bytes (0x04034b50)
...
typedef struct zip_file_header {
    libspectrum_dword magic;              // central file header signature   4 bytes (0x02014b50)
...
typedef struct zip_directory_info {
    libspectrum_dword magic;               // end of central dir signature   4 bytes (0x06054b50)
    libspectrum_word disk_index;           // number of this disk            2 bytes
    libspectrum_word directory_disk_index; // number of the disk with the  start of the central directory                                                                                  // 2 bytes
    libspectrum_word disk_file_count;      // total number of entries in the central directory on this disk 2 bytes
    libspectrum_word file_count;           // total number of entries in the central directory 2 bytes
    libspectrum_dword directory_size;      // size of the central directory 4 bytes
    libspectrum_dword directory_offset;    // offset of start of central directory with respect to the
                                           // starting disk number 4 bytes
    libspectrum_word comment_size;         // .ZIP file comment length 2 bytes
    // libspectrum_byte comment[comment_size];
} zip_directory_info;
...
libspectrum_error libspectrum_zip_next(struct libspectrum_zip *zip, zip_stat *info);

libspectrum_error libspectrum_zip_read(struct libspectrum_zip *zip, libspectrum_byte **buffer, size_t *size);

int libspectrum_zip_locate(struct libspectrum_zip *zip, const char *filename, int flags, zip_stat *info);

libspectrum_error libspectrum_zip_rewind(struct libspectrum_zip *zip);

void libspectrum_zip_close(struct libspectrum_zip *zip) ;

unsigned int libspectrum_zip_num_entries(struct libspectrum_zip *zip);
