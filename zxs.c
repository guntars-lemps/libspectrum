...
static libspectrum_error read_chunk(libspectrum_snap *snap, const libspectrum_byte **buffer, const libspectrum_byte *end);
...
static libspectrum_error read_riff_chunk(libspectrum_snap *snap,
                                         int *compression GCC_UNUSED,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end,
                                         size_t data_length GCC_UNUSED,
                                         int parameter GCC_UNUSED);

static libspectrum_error read_fmtz_chunk(libspectrum_snap *snap,
                                         int *compression,
                                         const libspectrum_byte **buffer,
...
static libspectrum_error read_rz80_chunk(libspectrum_snap *snap,
                                         int *compression GCC_UNUSED,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end GCC_UNUSED,
                                         size_t data_length,
...
static libspectrum_error read_r048_chunk(libspectrum_snap *snap,
                                         int *compression GCC_UNUSED,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end GCC_UNUSED,
                                         size_t data_length,
...
static libspectrum_error read_r128_chunk(libspectrum_snap *snap,
                                         int *compression GCC_UNUSED,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end GCC_UNUSED,
                                         size_t data_length,
...
static libspectrum_error read_rplus3_chunk(libspectrum_snap *snap,
                                           int *compression GCC_UNUSED,
                                           const libspectrum_byte **buffer,
                                           const libspectrum_byte *end GCC_UNUSED,
                                           size_t data_length,
...
static libspectrum_error read_ram_chunk(libspectrum_snap *snap,
                                        int *compression,
                                        const libspectrum_byte **buffer,
                                        const libspectrum_byte *end GCC_UNUSED,
                                        size_t data_length,
...
static libspectrum_error inflate_block(libspectrum_byte **uncompressed,
                                       size_t *uncompressed_length,
                                       const libspectrum_byte **compressed,
                                       size_t compressed_length)
{
    ...
    *uncompressed = libspectrum_new(libspectrum_byte, *uncompressed_length);
    ...
    error = uncompress(*uncompressed, &actual_length, zlib_buffer, (compressed_length + 6));

    /* At this point, we expect to get a Z_DATA_ERROR, as we don't have
       the Adler-32 checksum of the data. There is a 1 in 65521 chance that the random bytes will match,
       so we might (very rarely) get Z_OK
    */
    if ((error != Z_DATA_ERROR) && (error != Z_OK)) {
        libspectrum_free(*uncompressed);
        libspectrum_free(zlib_buffer);
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "zxs_inflate_block: unexpected zlib error");
                                         ...
                                         int parameter GCC_UNUSED)
{
    ...
    if ((end - *buffer) < 4) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "zxs_read_riff_chunk: not enough data for form type");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    *buffer += 4;
    ...
    if (strcmp(id, "SNAP")) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "zxs_read_riff_chunk: unknown form type '%s'", id);
        return LIBSPECTRUM_ERROR_UNKNOWN;
    }
                                         ...
                                         int parameter GCC_UNUSED)
{
    ...
    *buffer += 2; // Skip version number
    ...
    switch (model) {
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "zxs_read_fmtz_chunk: unknown machine type 0x%04x", model);
            return LIBSPECTRUM_ERROR_UNKNOWN;
    }
    ...
    *compression = libspectrum_read_word(buffer);
    ...
    switch (*compression) {
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN,
                                    "zxs_read_fmtz_chunk: unknown compression type 0x%04x",
                                    *compression);
            return LIBSPECTRUM_ERROR_UNKNOWN;
    }
                                         ...
                                         int parameter GCC_UNUSED)
{
    if (data_length != 9) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN,
                                "zxs_read_r048_chunk: unknown length %lu",
                                (unsigned long)data_length);
        return LIBSPECTRUM_ERROR_UNKNOWN;
    }

    libspectrum_snap_set_out_ula(snap, **buffer);
    (*buffer)++;
                                        ...
                                        int parameter)
{
    ...
    if (*compression) {
        ...
        if (uncompressed_length != 0x4000) {
            ...
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT,
                                    "zxs_read_ram_chunk: page %d does not expand to 0x4000 bytes",
                                    page);
        ...
        if (data_length != 0x4000) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN,
                                    "zxs_read_ram_chunk: page %d has unknown length %lu",
                                    page,
                                    (unsigned long)data_length);
            return LIBSPECTRUM_ERROR_UNKNOWN;
        }
...
static libspectrum_error read_chunk(libspectrum_snap *snap, const libspectrum_byte **buffer, const libspectrum_byte *end)
{
    ...
    if ((*buffer + data_length) > end) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "zxs_read_chunk: chunk length goes beyond end of file");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    if (!done) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "zxs_read_chunk: unknown chunk id '%s'", id);
        *buffer += data_length;
    }
...
libspectrum_error libspectrum_zxs_read(libspectrum_snap *snap, const libspectrum_byte *buffer, size_t length)
{
    libspectrum_error error;
