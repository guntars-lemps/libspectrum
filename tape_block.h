...
typedef struct libspectrum_tape_rom_block_state {
    ...
    libspectrum_byte current_byte; // The current data byte; gets shifted out as we read bits from it
...
/* A pause block - some formats use pause in ms, some use tstates.
   Fuse uses tstates but wants to be able to write back the original value to a file if re-saved so store both */
...
typedef struct libspectrum_tape_custom_block {
    ...
    // And the data for it; currently, no attempt is made to interpret this data
...
typedef struct libspectrum_tape_data_block_state {
    ...
    int bit_flags; // Any flags to be set when this bit is returned
    int level; // Mic level 0/1
...
struct libspectrum_tape_block_state {
    ...
    // Where to return to after a loop, and how many iterations of the loop to do
    GSList* loop_block;
    size_t loop_count;
