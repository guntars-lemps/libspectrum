...
/* The number of pilot pulses for the standard ROM loader NB:
   These disagree with the .tzx specification (they're 5 and 1 less respectively), but are correct.
   Entering the loop at #04D8 in the 48K ROM with HL == #0001 will produce the first sync pulse,
   not a pilot pulse, which gives a difference of one in both cases.
   The further difference of 4 in the header count is just a screw-up in the .tzx specification AFAICT */
...
static libspectrum_error rom_init(libspectrum_tape_rom_block *block, libspectrum_tape_rom_block_state *state);
static libspectrum_error turbo_init(libspectrum_tape_turbo_block *block, libspectrum_tape_turbo_block_state *state);
static libspectrum_error pure_data_init(libspectrum_tape_pure_data_block *block,
                                        libspectrum_tape_pure_data_block_state *state);
static void raw_data_init(libspectrum_tape_raw_data_block *block, libspectrum_tape_raw_data_block_state *state);
static libspectrum_error generalised_data_init(libspectrum_tape_generalised_data_block *block,
                                               libspectrum_tape_generalised_data_block_state *state);
static libspectrum_error data_block_init(libspectrum_tape_data_block *block, libspectrum_tape_data_block_state *state);


libspectrum_tape_block *libspectrum_tape_block_alloc(libspectrum_tape_type type)
{
...
static void free_symbol_table(libspectrum_tape_generalised_data_symbol_table *table)
{
    size_t i;
...
libspectrum_tape_type libspectrum_tape_block_type(libspectrum_tape_block *block)
{
    return block->type;
}


libspectrum_error libspectrum_tape_block_set_type(libspectrum_tape_block *block, libspectrum_tape_type type)
{
    block->type = type;
    return LIBSPECTRUM_ERROR_NONE;
}
...
libspectrum_error libspectrum_tape_block_init(libspectrum_tape_block *block, libspectrum_tape_block_state *state)
{
    if (!block) {
        return LIBSPECTRUM_ERROR_NONE;
    }
    ...
    switch (libspectrum_tape_block_type(block)) {
        ...
        case LIBSPECTRUM_TAPE_BLOCK_PULSES:
            ...
            break;
        case LIBSPECTRUM_TAPE_BLOCK_PURE_DATA:
            return pure_data_init(&(block->types.pure_data), &(state->block_state.pure_data));
        case LIBSPECTRUM_TAPE_BLOCK_RAW_DATA:
            raw_data_init(&(block->types.raw_data), &(state->block_state.raw_data));
            return LIBSPECTRUM_ERROR_NONE;
        case LIBSPECTRUM_TAPE_BLOCK_GENERALISED_DATA:
            return generalised_data_init(&(block->types.generalised_data), &(state->block_state.generalised_data));
        case LIBSPECTRUM_TAPE_BLOCK_RLE_PULSE:
        ...
        case LIBSPECTRUM_TAPE_BLOCK_PULSE_SEQUENCE:
            ...
            return LIBSPECTRUM_ERROR_NONE;
        case LIBSPECTRUM_TAPE_BLOCK_DATA_BLOCK:
            return data_block_init(&(block->types.data_block), &(state->block_state.data_block));
...
static libspectrum_error rom_init(libspectrum_tape_rom_block *block, libspectrum_tape_rom_block_state *state)
{
    ...
    state->edge_count = (block->length && (block->data[0] & 0x80)) ?
                            LIBSPECTRUM_TAPE_PILOTS_DATA : LIBSPECTRUM_TAPE_PILOTS_HEADER;
...
static libspectrum_error data_block_init(libspectrum_tape_data_block *block, libspectrum_tape_data_block_state *state)
{
    ...
    // If both bit0 and bit1 are two matching pulses,
    // then assign them to be flagged as short/long pulses respectively
    if ((block->bit0_pulse_count == 2) && (block->bit1_pulse_count == 2) &&
        (block->bit0_pulses[0] == block->bit0_pulses[1]) && (block->bit1_pulses[0] == block->bit1_pulses[1]) &&
...
libspectrum_error libspectrum_tape_block_read_symbol_table_parameters(libspectrum_tape_block *block,
                                                                      int pilot,
                                                                      const libspectrum_byte **ptr)
{
    libspectrum_tape_generalised_data_block *generalised = &block->types.generalised_data;

    libspectrum_tape_generalised_data_symbol_table *table = pilot ? &generalised->pilot_table : &generalised->data_table;
...
libspectrum_error libspectrum_tape_block_read_symbol_table(libspectrum_tape_generalised_data_symbol_table *table,
                                                           const libspectrum_byte **ptr,
                                                           size_t length)
{
    if (table->symbols_in_block) {
        ...
        table->symbols = libspectrum_new(libspectrum_tape_generalised_data_symbol, table->symbols_in_table);

        for (i = 0, symbol = table->symbols; i < table->symbols_in_table; i++, symbol++) {
            symbol->edge_type = **ptr;
            (*ptr)++;
...
static int libspectrum_bits_set_n_bits(libspectrum_byte byte, libspectrum_byte bits)
{
    int i;
    int retval = 0;
...
void libspectrum_init_bits_set(void)
{
    ...
    for (i = 0; i < BITS_SET_ARRAY_SIZE; i++) {
        bits_set[i] = libspectrum_bits_set_n_bits((libspectrum_byte)i, LIBSPECTRUM_BITS_IN_BYTE);
    }
}


static libspectrum_dword convert_pulses_to_tstates(libspectrum_dword set_bit_length,
                                                   libspectrum_dword unset_bit_length,
                                                   int bits_set_in_block,
                                                   int bit_block_size)
{
...
static libspectrum_dword rom_block_length(libspectrum_tape_rom_block *rom)
{
...
static libspectrum_dword turbo_block_length(libspectrum_tape_turbo_block *turbo)
{
    libspectrum_dword length = (turbo->pilot_pulses * turbo->pilot_length) + turbo->sync1_length +
                               turbo->sync2_length + turbo->pause_tstates;
    size_t i;
    if (turbo->length) {
        int bits_set_in_last_byte = libspectrum_bits_set_n_bits(turbo->data[turbo->length - 1], turbo->bits_in_last_byte);
...
static libspectrum_dword pulses_block_length(libspectrum_tape_pulses_block *pulses)
{
    libspectrum_dword length = 0;
    size_t i;

    for (i = 0; i < pulses->count; i++) {
        length += pulses->lengths[i];
    }
...
static libspectrum_dword pure_data_block_length(libspectrum_tape_pure_data_block *pure_data)
{
    libspectrum_dword length = pure_data->pause_tstates;
    size_t i;
    if (pure_data->length) {
        int bits_set_in_last_byte = libspectrum_bits_set_n_bits(pure_data->data[pure_data->length - 1],
...
static libspectrum_dword raw_data_block_length(libspectrum_tape_raw_data_block *raw_data)
{
    libspectrum_dword length = raw_data->pause_tstates;

    length += ((LIBSPECTRUM_BITS_IN_BYTE * raw_data->length) -
               (LIBSPECTRUM_BITS_IN_BYTE - raw_data->bits_in_last_byte)) * raw_data->bit_length;

    return length;
}


static libspectrum_dword rle_pulse_block_length(libspectrum_tape_rle_pulse_block *rle_pulse)
{
    libspectrum_dword length = 0;
    size_t i;
...
static libspectrum_dword generalised_data_block_length(libspectrum_tape_generalised_data_block *generalised_data)
{
    libspectrum_dword length = 0;
    libspectrum_tape_generalised_data_block_state state;
    ...
    while (!end_of_block) {
        error = generalised_data_edge(generalised_data, &state, &tstates, &end_of_block, &flags);
        if (error) {
            return -1;
        }
...
static libspectrum_dword pulse_sequence_block_length(libspectrum_tape_pulse_sequence_block *pulses)
{
    libspectrum_dword length = 0;
    size_t i;
...
static libspectrum_dword data_block_length(libspectrum_tape_data_block *data_block)
{
    libspectrum_dword length = 0;
    size_t i;
    if (data_block->count) {
        int bits_set_in_last_byte = libspectrum_bits_set_n_bits(data_block->data[data_block->length - 1],
        ...
        bit0_length /= data_block->bit0_pulse_count ? data_block->bit0_pulse_count : 1;

        for (i = 0; i < data_block->bit1_pulse_count; i++) {
            bit1_length += data_block->bit1_pulses[i];
        }
        bit1_length /= data_block->bit1_pulse_count ? data_block->bit1_pulse_count : 1;
