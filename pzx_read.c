...
static libspectrum_error read_pzxt_block(libspectrum_tape *tape,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end,
                                         size_t data_length,
                                         pzx_context *ctx);

static libspectrum_error read_puls_block(libspectrum_tape *tape,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end,
                                         size_t data_length,
                                         pzx_context *ctx);

static libspectrum_error read_data_block(libspectrum_tape *tape,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end,
                                         size_t data_length,
                                         pzx_context *ctx);

static libspectrum_error read_paus_block(libspectrum_tape *tape,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end,
                                         size_t data_length,
                                         pzx_context *ctx);

static libspectrum_error read_brws_block(libspectrum_tape *tape,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end,
                                         size_t data_length,
                                         pzx_context *ctx);

static libspectrum_error read_stop_block(libspectrum_tape *tape,
                                         const libspectrum_byte **buffer,
                                         const libspectrum_byte *end,
                                         size_t data_length,
                                         pzx_context *ctx);

static libspectrum_error skip_block(libspectrum_tape *tape GCC_UNUSED,
                                    const libspectrum_byte **buffer,
...
static libspectrum_error read_block(libspectrum_tape *tape,
                                    const libspectrum_byte **buffer,
                                    const libspectrum_byte *end,
                                    pzx_context *ctx);

static libspectrum_error pzx_read_data(const libspectrum_byte **ptr,
                                       const libspectrum_byte *end,
                                       size_t length,
                                       libspectrum_byte **data);

static libspectrum_error pzx_read_string(const libspectrum_byte **ptr, const libspectrum_byte *end, char **dest);



static int info_t_compar(const void *a, const void *b)
{
    const char *key = a;
...
static int get_id_byte(char *info_tag)
{
    struct info_t *info = (struct info_t*)bsearch(info_tag,
                                                  info_ids,
                                                  ARRAY_SIZE(info_ids),
                                         ...
                                         pzx_context *ctx)
{
    ...
    ctx->version = (**buffer) << 8;
    (*buffer)++;
    ctx->version |= **buffer;
    (*buffer)++;

    if ((ctx->version < 0x0100) || (ctx->version >= 0x0200)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "read_pzxt_block: only version 1 pzx files are supported");
    ...
    while (*buffer < block_end) {
        ...
        if (id == -1) {
            size_t new_len = strlen(info_tag) + strlen(string) +  strlen(": ") + 1;
    ...
    if (count) {
        libspectrum_tape_block *block = libspectrum_tape_block_alloc(LIBSPECTRUM_TAPE_BLOCK_ARCHIVE_INFO);
                                         ...
                                         pzx_context *ctx)
{
    ...
    if (data_length < 8) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_data_block: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    bits_in_last_byte = (count % LIBSPECTRUM_BITS_IN_BYTE) ? (count % LIBSPECTRUM_BITS_IN_BYTE) : LIBSPECTRUM_BITS_IN_BYTE;
    ...
    error = pzx_read_data(buffer, block_end, (p0_count * sizeof(libspectrum_word)), ((libspectrum_byte**) & p0_pulses));
    if (error) {
        return error;
    }

    error = pzx_read_data(buffer, block_end, (p1_count * sizeof(libspectrum_word)), ((libspectrum_byte**) & p1_pulses));
    if (error) {
...
pzx_corrupt:
    libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_next_pulse: not enough data in buffer");
    return LIBSPECTRUM_ERROR_CORRUPT;
}
                                         ...
                                         pzx_context *ctx)
{
    ...
    size_t *pulse_repeats_buffer = libspectrum_new(size_t, buffer_sizes);
    ...
    while ((block_end - (*buffer)) > (ptrdiff_t)0) {
        ...
        if (buffer_sizes == count) {
            buffer_sizes *= 2;
            pulse_repeats_buffer = libspectrum_renew(size_t, pulse_repeats_buffer, buffer_sizes);
            lengths_buffer = libspectrum_renew(libspectrum_dword, lengths_buffer, buffer_sizes);
        }
    }

    if (count == 0) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_puls_block: no pulses found in pulse block");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    if (buffer_sizes != count) {
        pulse_repeats_buffer = libspectrum_renew(size_t, pulse_repeats_buffer, count);
        lengths_buffer = libspectrum_renew(libspectrum_dword, lengths_buffer, count);
    }
                                         ...
                                         pzx_context *ctx)
{
    ...
    if (data_length < 2) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_paus_block: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
...
static libspectrum_error read_block_header(char *id,
                                           libspectrum_dword *data_length,
                                           const libspectrum_byte **buffer,
                                           const libspectrum_byte *end)
{
    if ((end - *buffer) < 8) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_block_header: not enough data for block header");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
                                    ...
                                    pzx_context *ctx)
{
    ...
    if ((end - *buffer) < data_length) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "read_block: block length goes beyond end of file");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    if (!done) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "read_block: unknown block id '%s'", id);
        *buffer += data_length;
    }
...
libspectrum_error internal_pzx_read(libspectrum_tape *tape, const libspectrum_byte *buffer, size_t length)
{
    libspectrum_error error;
    ...
    if ((end - buffer) < 8) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "internal_pzx_read: not enough data for PZX header");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    if (memcmp(buffer, signature, signature_length)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_SIGNATURE, "internal_pzx_read: wrong signature");
                                       ...
                                       libspectrum_byte **data)
{
    ...
    if ((end - (*ptr)) < (ptrdiff_t)(length)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "pzx_read_data: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    // Allocate memory for the data; the check for *length is to avoid the implementation-defined behaviour of malloc(0)
    if (length) {
