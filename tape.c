...
const int LIBSPECTRUM_TAPE_FLAGS_STOP48       = 1 << 2; // Stop tape if in 48K mode
...
const int LIBSPECTRUM_TAPE_FLAGS_LENGTH_SHORT = 1 << 6; // Short edge; used for loader acceleration
const int LIBSPECTRUM_TAPE_FLAGS_LENGTH_LONG  = 1 << 7; // Long edge; used for loader acceleration
const int LIBSPECTRUM_TAPE_FLAGS_TAPE         = 1 << 8; // End of tape

/*** Local function prototypes ***/

// Free the memory used by a block
static void block_free(gpointer data, gpointer user_data);

// Functions to get the next edge

static libspectrum_error rom_edge(libspectrum_tape_rom_block *block,
                                  libspectrum_tape_rom_block_state *state,
                                  libspectrum_dword *tstates,
                                  int *end_of_block,
                                  int *flags);
static libspectrum_error rom_next_bit(libspectrum_tape_rom_block *block, libspectrum_tape_rom_block_state *state);

static libspectrum_error turbo_edge(libspectrum_tape_turbo_block *block,
                                    libspectrum_tape_turbo_block_state *state,
                                    libspectrum_dword *tstates,
                                    int *end_of_block,
                                    int *flags);
static libspectrum_error turbo_next_bit(libspectrum_tape_turbo_block *block,
                                        libspectrum_tape_turbo_block_state *state);

static libspectrum_error tone_edge(libspectrum_tape_pure_tone_block *block,
                                   libspectrum_tape_pure_tone_block_state *state,
                                   libspectrum_dword *tstates,
                                   int *end_of_block);

static libspectrum_error pulses_edge(libspectrum_tape_pulses_block *block,
                                     libspectrum_tape_pulses_block_state *state,
                                     libspectrum_dword *tstates,
                                     int *end_of_block);

static libspectrum_error pure_data_edge(libspectrum_tape_pure_data_block *block,
                                        libspectrum_tape_pure_data_block_state *state,
                                        libspectrum_dword *tstates,
                                        int *end_of_block,
                                        int *flags);

static libspectrum_error raw_data_edge(libspectrum_tape_raw_data_block *block,
                                       libspectrum_tape_raw_data_block_state *state,
                                       libspectrum_dword *tstates,
                                       int *end_of_block,
                                       int *flags);

static libspectrum_error jump_blocks(libspectrum_tape *tape, int offset);

static libspectrum_error rle_pulse_edge(libspectrum_tape_rle_pulse_block *block,
                                        libspectrum_tape_rle_pulse_block_state *state,
                                        libspectrum_dword *tstates,
                                        int *end_of_block);

static libspectrum_error pulse_sequence_edge(libspectrum_tape_pulse_sequence_block *block,
                                             libspectrum_tape_pulse_sequence_block_state *state,
                                             libspectrum_dword *tstates,
                                             int *end_of_block,
                                             int *flags);

libspectrum_error libspectrum_tape_data_block_next_bit(libspectrum_tape_data_block *block,
                                                       libspectrum_tape_data_block_state *state);

static libspectrum_error data_block_edge(libspectrum_tape_data_block *block,
                                         libspectrum_tape_data_block_state *state,
                                         libspectrum_dword *tstates,
                                         int *end_of_block,
                                         int *flags);
...
static void block_free(gpointer data, gpointer user_data GCC_UNUSED)
{
...
libspectrum_error libspectrum_tape_read(libspectrum_tape *tape,
                                        ...
                                        const char *filename)
{
    ...
    switch (type) {
        ...
        case LIBSPECTRUM_ID_TAPE_LTP:
            error = internal_tap_read(tape, buffer, length, type);
            break;

        case LIBSPECTRUM_ID_TAPE_TZX:
            error = internal_tzx_read(tape, buffer, length);
            break;

        case LIBSPECTRUM_ID_TAPE_WARAJEVO:
            error = internal_warajevo_read(tape, buffer, length);
            break;

        case LIBSPECTRUM_ID_TAPE_Z80EM:
            error = libspectrum_z80em_read(tape, buffer, length);
            break;

        case LIBSPECTRUM_ID_TAPE_CSW:
            error = libspectrum_csw_read(tape, buffer, length);
            break;

        case LIBSPECTRUM_ID_TAPE_WAV:
    #ifdef HAVE_LIB_AUDIOFILE
            error = libspectrum_wav_read(tape, filename);
            break;
        ...
        case LIBSPECTRUM_ID_TAPE_PZX:
            error = internal_pzx_read(tape, buffer, length);
            break;

        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_tape_read: not a tape file");
...
libspectrum_error libspectrum_tape_write(libspectrum_byte **buffer,
                                         size_t *length,
                                         ...
                                         libspectrum_id_t type)
{
    ...
    if (class != LIBSPECTRUM_CLASS_TAPE) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_INVALID, "libspectrum_tape_write: not a tape format");
    ...
    switch (type) {
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_UNKNOWN, "libspectrum_tape_write: format not supported");
...
libspectrum_error libspectrum_tape_get_next_edge_internal(libspectrum_dword *tstates,
                                                          int *flags,
                                                          libspectrum_tape *tape,
                                                          libspectrum_tape_block_state *it)
{
    ...
    libspectrum_tape_block *block = libspectrum_tape_iterator_current(it->current_block);
    ...
    if (block) {
        switch (block->type) {
            case LIBSPECTRUM_TAPE_BLOCK_ROM:
                error = rom_edge(&(block->types.rom), &(it->block_state.rom), tstates, &end_of_block, flags);
            if (error) {
                return error;
            }
            break;
        case LIBSPECTRUM_TAPE_BLOCK_TURBO:
            error = turbo_edge(&(block->types.turbo), &(it->block_state.turbo), tstates, &end_of_block, flags);
            if (error) {
                return error;
            }
            break;
        case LIBSPECTRUM_TAPE_BLOCK_PURE_TONE:
            error = tone_edge(&(block->types.pure_tone), &(it->block_state.pure_tone), tstates, &end_of_block);
            if (error) {
                return error;
            }
            break;
        case LIBSPECTRUM_TAPE_BLOCK_PULSES:
            error = pulses_edge(&(block->types.pulses), &(it->block_state.pulses), tstates, &end_of_block);
            if (error) {
                return error;
            }
            break;
        ...
        case LIBSPECTRUM_TAPE_BLOCK_RAW_DATA:
            error = raw_data_edge(&(block->types.raw_data),
                                  &(it->block_state.raw_data),
                                  tstates,
                                  &end_of_block,
                                  flags);
            if (error) {
        ...
        case LIBSPECTRUM_TAPE_BLOCK_PAUSE:
            *tstates = block->types.pause.length_tstates; end_of_block = 1;
            // If the pause isn't a "don't care" level then set the appropriate pulse level
            if ((block->types.pause.level != -1) && block->types.pause.length_tstates) {
                *flags |= block->types.pause.level ?
        ...
        case LIBSPECTRUM_TAPE_BLOCK_JUMP:
            ...
            *tstates = 0;
            *flags |= LIBSPECTRUM_TAPE_FLAGS_NO_EDGE; end_of_block = 1;
            no_advance = 1;
            break;
        ...
        case LIBSPECTRUM_TAPE_BLOCK_SET_SIGNAL_LEVEL:
            *tstates = 0;
            end_of_block = 1;
            // Inverted as the following block will flip the level before recording the edge
        ...
        case LIBSPECTRUM_TAPE_BLOCK_RLE_PULSE:
            error = rle_pulse_edge(&(block->types.rle_pulse), &(it->block_state.rle_pulse), tstates, &end_of_block);
            if (error) {
                return error;
            }
            break;
    ...
    if (end_of_block) {
        ...
        *flags |= LIBSPECTRUM_TAPE_FLAGS_BLOCK;
        ...
        if (!no_advance) {
            ...
            // If we've just hit the end of the tape, stop the tape (and then 'rewind' to the start)
        ...
        error = libspectrum_tape_block_init(libspectrum_tape_iterator_current(it->current_block), it);
        if (error) {
            return error;
        }
...
/* The main function: called with a tape object and returns the number of
   t-states until the next edge, and a marker if this was the last edge on the tape */
libspectrum_error libspectrum_tape_get_next_edge(libspectrum_dword *tstates, int *flags, libspectrum_tape *tape)
{
    return libspectrum_tape_get_next_edge_internal(tstates, flags, tape, &(tape->state));
}
...
static void do_tail_pause(libspectrum_dword *tstates, int *end_of_block, int *flags)
{
    *end_of_block = 1;
    if (*tstates == 0) {
    ...
    int error;
        ...
        case LIBSPECTRUM_TAPE_STATE_SYNC2:
            ...
            *tstates = LIBSPECTRUM_TAPE_TIMING_SYNC2;
            ...
            error = rom_next_bit(block, state);
            if (error) {
                return error;
            }
            break;
        ...
        case LIBSPECTRUM_TAPE_STATE_DATA1:
            ...
            *flags |= (state->bit_tstates == LIBSPECTRUM_TAPE_TIMING_DATA0) ?
                          LIBSPECTRUM_TAPE_FLAGS_LENGTH_SHORT : LIBSPECTRUM_TAPE_FLAGS_LENGTH_LONG;
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "rom_edge: unknown state %d", state->state);
            return LIBSPECTRUM_ERROR_LOGIC;
...
static libspectrum_error rom_next_bit(libspectrum_tape_rom_block *block, libspectrum_tape_rom_block_state *state)
{
    ...
    if (++(state->bits_through_byte) == 8) {
        ...
        // If so, have we finished the entire block? If so, all we've got left after this is the pause at the end
        if (++(state->bytes_through_block) == block->length) {
            state->state = LIBSPECTRUM_TAPE_STATE_PAUSE;
            return LIBSPECTRUM_ERROR_NONE;
        }

        // If we've finished the current byte, but not the entire block, get the next byte
    ...
    state->bit_tstates = (next_bit ? LIBSPECTRUM_TAPE_TIMING_DATA1 : LIBSPECTRUM_TAPE_TIMING_DATA0);
    ...
    switch (state->state) {
        ...
        case LIBSPECTRUM_TAPE_STATE_SYNC2:
            ...
            *tstates = block->sync2_length;
            // Followed by the first bit of data
            error = turbo_next_bit(block, state);
            if (error) {
                return error;
            }
            break;
        ...
        case LIBSPECTRUM_TAPE_STATE_DATA1:
            // The first edge for a bit of data
            *tstates = state->bit_tstates;
            *flags |= (state->bit_tstates == block->bit0_length) ?
                          LIBSPECTRUM_TAPE_FLAGS_LENGTH_SHORT : LIBSPECTRUM_TAPE_FLAGS_LENGTH_LONG;
            // Followed by the second edge
            state->state = LIBSPECTRUM_TAPE_STATE_DATA2;
            break;
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "turbo_edge: unknown state %d", state->state);
            return LIBSPECTRUM_ERROR_LOGIC;
...
static libspectrum_error turbo_next_bit(libspectrum_tape_turbo_block *block, libspectrum_tape_turbo_block_state *state)
{
    int next_bit;
    ...
    if (++(state->bits_through_byte) == 8) {
        ...
        // If we're looking at the last byte, take account of the fact it may have less than 8 bits in it
                                        ...
                                        int *flags)
{
    ...
    switch (state->state) {
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "pure_data_edge: unknown state %d", state->state);
            return LIBSPECTRUM_ERROR_LOGIC;
...
libspectrum_error libspectrum_tape_pure_data_next_bit(libspectrum_tape_pure_data_block *block,
                                                      libspectrum_tape_pure_data_block_state *state)
{
    int next_bit;
    ...
    switch (state->state) {
        case LIBSPECTRUM_TAPE_STATE_DATA1:
            *tstates = state->bit_tstates;
            ...
            // Backwards as last bit is the state of the next bit as it has already been updated
            *flags |= state->last_bit ?
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "raw_edge: unknown state %d", state->state);
            return LIBSPECTRUM_ERROR_LOGIC;
    }
...
void libspectrum_tape_raw_data_next_bit(libspectrum_tape_raw_data_block *block,
                                        libspectrum_tape_raw_data_block_state *state)
{
    int length = 0;
    ...
    } while (((block->data[state->bytes_through_block] << state->bits_through_byte) & 0x80) != state->last_bit);
...
static libspectrum_byte get_generalised_data_bit(libspectrum_tape_generalised_data_block *block,
...
libspectrum_byte get_generalised_data_symbol(libspectrum_tape_generalised_data_block *block,
                                             libspectrum_tape_generalised_data_block_state *state)
{
    libspectrum_byte symbol;
    size_t i;

    for (i = 0, symbol = 0; i < block->bits_per_data_symbol; i++) {
        symbol <<= 1;
...
static void set_tstates_and_flags(libspectrum_tape_generalised_data_symbol *symbol,
                                  libspectrum_byte edge,
                                  libspectrum_dword *tstates,
                                  int *flags)
{
...
libspectrum_error generalised_data_edge(libspectrum_tape_generalised_data_block *block,
                                        libspectrum_tape_generalised_data_block_state *state,
                                        libspectrum_dword *tstates,
                                        int *end_of_block,
                                        int *flags)
{
    ...
    switch (state->state) {
        case LIBSPECTRUM_TAPE_STATE_PILOT:
            ...
            set_tstates_and_flags(symbol, state->edges_through_symbol, tstates, flags);
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "%s: unknown state %d", __func__, state->state);
            return LIBSPECTRUM_ERROR_LOGIC;
    }
...
static libspectrum_error jump_blocks(libspectrum_tape *tape, int offset)
{
    ...
    current_position = g_slist_position(tape->blocks, tape->state.current_block);
    ...
    if (block->data[state->index]) {
        ...
        *tstates = block->scale * block->data[state->index++];
        ...
        if ((state->index + 5) > block->length) {
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "rle_pulse_edge: file is truncated\n");
            return LIBSPECTRUM_ERROR_LOGIC;
        }
                                             ...
                                             int *flags)
{
    ...
    // Get the length of this edge
    *tstates = 0;
    /* Skip past any 0 blocks until we find a non 0 block or reach the end of the block,
       keeping track of the current mic level */
    ...
    if (new_level != state->level) {
        *flags |= new_level ? LIBSPECTRUM_TAPE_FLAGS_LEVEL_HIGH : LIBSPECTRUM_TAPE_FLAGS_LEVEL_LOW;
        ...
        // If the net effect of this edge was 0 tstates and no level change, it was much ado about nothing
        *flags |= LIBSPECTRUM_TAPE_FLAGS_NO_EDGE;
    }
                                                       ...
                                                       libspectrum_tape_data_block_state *state)
{
    ...
    if (++(state->bits_through_byte) == 8) {
        ...
        // If so, have we finished the entire block? If so, all we've got left after this is the tail at the end
        if (++(state->bytes_through_block) == block->length) {
    ...
    state->pulse_count = next_bit ? block->bit1_pulse_count : block->bit0_pulse_count;
                                         ...
                                         int *flags)
{
    ...
    switch (state->state) {
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "data_block_edge: unknown state %d", state->state);
            return LIBSPECTRUM_ERROR_LOGIC;

    }

    if (!(*flags & LIBSPECTRUM_TAPE_FLAGS_NO_EDGE)) {
        *flags |= state->level ? LIBSPECTRUM_TAPE_FLAGS_LEVEL_HIGH : LIBSPECTRUM_TAPE_FLAGS_LEVEL_LOW;
...
libspectrum_tape_block WIN32_DLL * libspectrum_tape_peek_last_block(libspectrum_tape *tape)
{
...
libspectrum_error libspectrum_tape_position(int *n, libspectrum_tape *tape)
{
    *n = g_slist_position(tape->blocks, tape->state.current_block);
    ...
    if (*n == -1) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_LOGIC, "libspectrum_tape_position: current block is not in tape!");
        return LIBSPECTRUM_ERROR_LOGIC;
    }
...
libspectrum_error libspectrum_tape_nth_block(libspectrum_tape *tape, int n)
{
    ...
    if (!new_block) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_tape_nth_block: tape does not have block %d", n);
        return LIBSPECTRUM_ERROR_CORRUPT;
    }

    tape->state.current_block = new_block;

    error = libspectrum_tape_block_init(tape->state.current_block->data, &(tape->state));
    if (error) {
        return error;
    }
...
void libspectrum_tape_append_block(libspectrum_tape *tape, libspectrum_tape_block *block)
{
    if (tape->blocks == NULL) {
        ...
        tape->last_block = g_slist_append(tape->last_block, (gpointer)block)->next;
    }

    /* If we previously didn't have a tape loaded (implied by tape->current_block == NULL),
       set up so that we point to the start of the tape */
    if (!tape->state.current_block) {
...
/* Given a tape file, attempt to guess which sort of hardware it should run on by looking for a hardware block (0x33)
   Deliberately not mentioned in libspectrum.h(.in) as I'm really not sure this function is actually useful */
libspectrum_error libspectrum_tape_guess_hardware(libspectrum_machine *machine, const libspectrum_tape *tape)
{
    GSList *ptr;
    int score, current_score;
    size_t i;

    *machine = LIBSPECTRUM_MACHINE_UNKNOWN;
    current_score = 0;
    ...
    for (ptr = tape->blocks; ptr; ptr = ptr->next) {
        ...
        if (block->type != LIBSPECTRUM_TAPE_BLOCK_HARDWARE) {
            continue;
        }
        ...
        for (i = 0; i < hardware->count; i++) {
            ...
            switch (hardware->ids[i]) {
                ...
                case 0: // 16K Spectrum
                    *machine = LIBSPECTRUM_MACHINE_16;
                    current_score = score;
                    break;
                ...
                case 5: // +2A and +3. Gee, thanks to whoever designed the TZX format for distinguishing those :-(
                    *machine = LIBSPECTRUM_MACHINE_PLUS3;
                    current_score = score;
                    break;

                case 6: // TC2048
                    *machine = LIBSPECTRUM_MACHINE_TC2048;
                    current_score = score;
                    break;
...
libspectrum_tape_block *libspectrum_tape_block_internal_init(libspectrum_tape_block_state *it, libspectrum_tape *tape)
{
    if (!tape || !tape->blocks) {
        return NULL;
    }
    ...
    if (libspectrum_tape_block_init(it->current_block->data, it)) {
        return NULL;
    }
...
libspectrum_tape_block *libspectrum_tape_iterator_peek_next(libspectrum_tape_iterator iterator)
{
    if (iterator) {
...
libspectrum_tape_state_type libspectrum_tape_state(libspectrum_tape *tape)
{
    ...
    switch (block->type) {
        ...
        case LIBSPECTRUM_TAPE_BLOCK_PURE_DATA:
            return tape->state.block_state.pure_data.state;
        case LIBSPECTRUM_TAPE_BLOCK_RAW_DATA:
            return tape->state.block_state.raw_data.state;
        case LIBSPECTRUM_TAPE_BLOCK_ROM:
            return tape->state.block_state.rom.state;
        case LIBSPECTRUM_TAPE_BLOCK_TURBO:
            return tape->state.block_state.turbo.state;

        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_INVALID,
                                    "invalid current block type 0x%02x in tape given to %s",
                                    block->type,
                                    __func__);
...
libspectrum_error libspectrum_tape_set_state(libspectrum_tape *tape, libspectrum_tape_state_type state)
{
    ...
    switch (block->type) {
        ...
        case LIBSPECTRUM_TAPE_BLOCK_PURE_DATA:
            tape->state.block_state.pure_data.state = state;
            break;
        case LIBSPECTRUM_TAPE_BLOCK_RAW_DATA:
            tape->state.block_state.raw_data.state = state;
            break;
        case LIBSPECTRUM_TAPE_BLOCK_ROM:
            tape->state.block_state.rom.state = state;
            break;
        case LIBSPECTRUM_TAPE_BLOCK_TURBO:
            tape->state.block_state.turbo.state = state;
            break;

        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_INVALID,
                                    "invalid current block type 0x%2x in tape given to %s",
                                    block->type,
                                    __func__);
            return LIBSPECTRUM_ERROR_INVALID;
    }
