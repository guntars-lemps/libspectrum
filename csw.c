...
libspectrum_error libspectrum_csw_read(libspectrum_tape *tape, const libspectrum_byte *buffer, size_t length)
{
    ...
    if (memcmp(csw_signature, buffer, signature_length)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_SIGNATURE, "libspectrum_csw_read: wrong signature");
    ...
    switch (buffer[0]) {
        ...
        case 2:
            ...
            csw_block->scale = buffer[2] | (buffer[3] << 8) | (buffer[4] << 16) | (buffer[5] << 24);
        ...
        default:
            libspectrum_print_error(LIBSPECTRUM_ERROR_MEMORY, "libspectrum_csw_read: unknown CSW version");
            return LIBSPECTRUM_ERROR_SIGNATURE;
    }
...
csw_bad_compress:
    ...
    libspectrum_print_error(LIBSPECTRUM_ERROR_MEMORY, "libspectrum_csw_read: unknown compression type");
    return LIBSPECTRUM_ERROR_CORRUPT;

csw_short:
    libspectrum_free(block);
    libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_csw_read: not enough data in buffer");
    return LIBSPECTRUM_ERROR_CORRUPT;
...
static libspectrum_dword find_sample_rate(libspectrum_tape *tape)
{
    ...
    /* FIXME: If tape has only one block that is a sampled type, just use it's rate and if it RLE,
       we should just zlib and write */
    for (block = libspectrum_tape_iterator_init(&iterator, tape);
        ...
        switch (libspectrum_tape_block_type(block)) {
            ...
            case LIBSPECTRUM_TAPE_BLOCK_RLE_PULSE:
                ;
                libspectrum_dword block_rate = 3500000 / libspectrum_tape_block_bit_length(block);

                if (found) {
                    if (block_rate != sample_rate) {
                        libspectrum_print_error(LIBSPECTRUM_ERROR_WARNING,
                                                "find_sample_rate: converting tape with mixed sample rates; "
                                                    "conversion may well not work");
                    }
                }
...
static libspectrum_error csw_write_body(libspectrum_buffer *buffer,
...
libspectrum_error libspectrum_csw_write(libspectrum_buffer *new_buffer, libspectrum_tape *tape)
{
    ...
    error = csw_write_body(body_buffer, tape, sample_rate, &body_uncompressed_length);
    if (error != LIBSPECTRUM_ERROR_NONE) {
        goto exit;
    }
    ...
    // Store where the total number of pulses (after decompression) will be written, and skip over those bytes
