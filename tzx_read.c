...
static libspectrum_error tzx_read_rom_block(libspectrum_tape *tape,
                                            const libspectrum_byte **ptr,
                                            const libspectrum_byte *end);
static libspectrum_error tzx_read_turbo_block(libspectrum_tape *tape,
                                              const libspectrum_byte **ptr,
                                              const libspectrum_byte *end);
static libspectrum_error tzx_read_pure_tone(libspectrum_tape *tape,
                                            const libspectrum_byte **ptr,
                                            const libspectrum_byte *end);
static libspectrum_error tzx_read_pulses_block(libspectrum_tape *tape,
                                               const libspectrum_byte **ptr,
                                               const libspectrum_byte *end);
static libspectrum_error tzx_read_pure_data(libspectrum_tape *tape,
                                            const libspectrum_byte **ptr,
                                            const libspectrum_byte *end);
static libspectrum_error tzx_read_raw_data(libspectrum_tape *tape,
                                           const libspectrum_byte **ptr,
                                           const libspectrum_byte *end);
static libspectrum_error tzx_read_generalised_data(libspectrum_tape *tape,
                                                   const libspectrum_byte **ptr,
                                                   const libspectrum_byte *end);
static libspectrum_error tzx_read_pause(libspectrum_tape *tape,
                                        const libspectrum_byte **ptr,
                                        const libspectrum_byte *end);
static libspectrum_error tzx_read_group_start(libspectrum_tape *tape,
                                              const libspectrum_byte **ptr,
                                              const libspectrum_byte *end);
static libspectrum_error tzx_read_jump(libspectrum_tape *tape,
                                       const libspectrum_byte **ptr,
                                       const libspectrum_byte *end);
static libspectrum_error tzx_read_loop_start(libspectrum_tape *tape,
                                             const libspectrum_byte **ptr,
                                             const libspectrum_byte *end);
static libspectrum_error tzx_read_select(libspectrum_tape *tape,
                                         const libspectrum_byte **ptr,
                                         const libspectrum_byte *end);
static libspectrum_error tzx_read_stop(libspectrum_tape *tape,
                                       const libspectrum_byte **ptr,
                                       const libspectrum_byte *end);
static libspectrum_error tzx_read_set_signal_level(libspectrum_tape *tape,
                                                   const libspectrum_byte **ptr,
                                                   const libspectrum_byte *end);
static libspectrum_error tzx_read_comment(libspectrum_tape *tape,
                                          const libspectrum_byte **ptr,
                                          const libspectrum_byte *end);
static libspectrum_error tzx_read_message(libspectrum_tape *tape,
                                          const libspectrum_byte **ptr,
                                          const libspectrum_byte *end);
static libspectrum_error tzx_read_archive_info(libspectrum_tape *tape,
                                               const libspectrum_byte **ptr,
                                               const libspectrum_byte *end);
static libspectrum_error tzx_read_hardware(libspectrum_tape *tape,
                                           const libspectrum_byte **ptr,
                                           const libspectrum_byte *end);
static libspectrum_error tzx_read_custom(libspectrum_tape *tape,
                                         const libspectrum_byte **ptr,
                                         const libspectrum_byte *end);
static libspectrum_error tzx_read_concat(const libspectrum_byte **ptr,
                                         const libspectrum_byte *end);

static void tzx_read_empty_block(libspectrum_tape *tape, libspectrum_tape_type id);

static libspectrum_error tzx_read_data(const libspectrum_byte **ptr,
                                       const libspectrum_byte *end,
                                       size_t *length,
                                       int bytes,
                                       libspectrum_byte **data);
static libspectrum_error tzx_read_string(const libspectrum_byte **ptr,
                                         const libspectrum_byte *end,
                                         char **dest);
...
libspectrum_error internal_tzx_read(libspectrum_tape *tape, const libspectrum_byte *buffer, const size_t length)
{
    ...
    // Must be at least as many bytes as the signature, and the major/minor version numbers
    if (length < (signature_length + 2)) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "libspectrum_tzx_create: not enough data in buffer");
                                            ...
                                            const libspectrum_byte *end)
{
    ...
    // Check there's enough left in the buffer for the pause and the data length
    if ((end - (*ptr)) < 4) {
                                              ...
                                              const libspectrum_byte *end)
{
    ...
    if ((end - (*ptr)) < 18) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "tzx_read_turbo_block: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
    ...
    // Get the metadata
    libspectrum_tape_block_set_pilot_length(block, ((*ptr)[0] + ((*ptr)[1] * 0x100)));
    (*ptr) += 2;
    libspectrum_tape_block_set_sync1_length(block, ((*ptr)[0] + ((*ptr)[1] * 0x100)));
    (*ptr) += 2;
    libspectrum_tape_block_set_sync2_length(block, ((*ptr)[0] + ((*ptr)[1] * 0x100)));
    (*ptr) += 2;
    libspectrum_tape_block_set_bit0_length (block, ((*ptr)[0] + ((*ptr)[1] * 0x100)));
    (*ptr) += 2;
    libspectrum_tape_block_set_bit1_length (block, ((*ptr)[0] + ((*ptr)[1] * 0x100)));
    (*ptr) += 2;
    libspectrum_tape_block_set_pilot_pulses(block, ((*ptr)[0] + ((*ptr)[1] * 0x100)));
    (*ptr) += 2;
                                            ...
                                            const libspectrum_byte *end)
{
    ...
    libspectrum_tape_block_set_pulse_length(block, ((*ptr)[0] + ((*ptr)[1] * 0x100)));
    (*ptr) += 2;
                                               ...
                                               const libspectrum_byte *end)
{
    ...
    if ((*ptr) == end) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "tzx_read_pulses_block: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
                                            ...
                                            const libspectrum_byte *end)
{
    ...
    // Check there's enough left in the buffer for all the metadata
    if ((end - (*ptr)) < 10) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "tzx_read_pure_data: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
                                           ...
                                           const libspectrum_byte *end)
{
    ...
    // Get the metadata
    libspectrum_tape_block_set_bit_length(block, ((*ptr)[0] + ((*ptr)[1] * 0x100)));
                                              ...
                                              const libspectrum_byte *end)
{
    ...
    if ((*ptr) == end) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "tzx_read_group_start: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
                                             ...
                                             const libspectrum_byte *end)
{
    ...
    if ((end - (*ptr)) < 2) {
        libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "tzx_read_loop_start: not enough data in buffer");
        return LIBSPECTRUM_ERROR_CORRUPT;
    }
                                         ...
                                         const libspectrum_byte *end)
{
    ...
    // Check there's enough left in the buffer for the length and the count byte
    if ((end - (*ptr)) < 3) {
    ...
    for (i = 0; i < count; i++) {
        ...
        if ((end - (*ptr)) < 3) {
            for (j = 0; j < i; j++) {
                libspectrum_free(descriptions[j]);
            }
                                                   ...
                                                   const libspectrum_byte *end)
{
    ...
    libspectrum_tape_block_set_level(block, !!(**ptr));
    (*ptr)++;
                                          ...
                                          const libspectrum_byte *end)
{
    ...
    libspectrum_set_pause_ms(block, ((**ptr) * 1000));
    (*ptr)++;
                                               ...
                                               const libspectrum_byte *end)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if ((end - (*ptr)) < 2) {
            size_t j;
            for (j = 0; j < i; j++) {
                libspectrum_free(strings[j]);
            }
            libspectrum_free(strings);
            libspectrum_free(ids);
            libspectrum_free(block);
            libspectrum_print_error(LIBSPECTRUM_ERROR_CORRUPT, "tzx_read_archive_info: not enough data in buffer");
            return LIBSPECTRUM_ERROR_CORRUPT;
        }
...
// Concatenation block: just skip it entirely as it serves no useful purpose
static libspectrum_error tzx_read_concat(const libspectrum_byte **ptr, const libspectrum_byte *end)
{
    ...
    /* Check there's enough data left;
       the -1 here is because we've already read the first byte of the signature as the block ID */
...
{
    int i;
    libspectrum_dword multiplier = 0x01;
    size_t padding;
